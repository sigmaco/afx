...
_AVXINL void AfxCubemapMatrix_Direct3D(afxM4d m, afxNat face)
{
    afxError err = NIL;
    AfxAssert(m);
    AfxM4dZero(m);
...
_AVXINL void AfxCubemapMatrix_OpenGL(afxM4d m, afxNat face)
{
    afxError err = NIL;
    AfxAssert(m);
    AfxM4dZero(m);
...
_AVXINL void AfxComputeOrthographicMatrix(afxM4d m, afxV2d const extent, afxReal near, afxReal far, avxClipSpace const* clip)
{
    ...
    if (clip->nonRh)
    {
        AfxAssert(clip->boundary == avxClipBoundary_ZERO_TO_ONE); // Direct3D
