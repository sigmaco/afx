...
_AMX afxError AfxDrawSky(avxCmdb cmdb, akxSky* sky)
{
...
_AMX afxError AfxBuildSkybox(akxSky* sky, afxSimulation sim, afxDrawInput din)
{
    afxError err = NIL;
    ...
    AfxColorSet(sky->ambientColor, 0.1, 0.1, 0.1, 1);
    AfxColorSet(sky->emissiveColor, 0.1, 0.1, 0.1, 1);
