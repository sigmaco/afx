...
_AMX afxClass const* AfxGetPragmaDeviceClass(void)
{
...
_AMX afxClass const* AfxGetMathBridgeClass(afxPragmaDevice mdev)
{
    afxError err = AFX_ERR_NONE;
    /// mdev must be a valid afxPragmaDevice handle.
...
_AMX afxClass const* AfxGetSimulationClass(afxPragmaDevice mdev)
{
    afxError err = AFX_ERR_NONE;
    /// mdev must be a valid afxPragmaDevice handle.
    AfxAssertObjects(1, &mdev, afxFcc_MDEV);
...
_AMX afxNat AfxCountPragmaPorts(afxPragmaDevice mdev)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &mdev, afxFcc_MDEV);
    return mdev->portCnt;
...
_AMX afxBool AfxIsPragmaDevicePrompt(afxPragmaDevice mdev)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &mdev, afxFcc_MDEV);
...
_AMX afxError _AmxMdevDtorCb(afxPragmaDevice mdev)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &mdev, afxFcc_MDEV);

    AfxAssert(mdev->stopCb);

    if (mdev->stopCb(mdev))
        AfxThrowError();

    AfxAssert(!mdev->idd);
    AfxCleanUpChainedClasses(&mdev->dev.classes);
    ...
    AfxDeallocateInstanceData(mdev, AFX_COUNTOF(stashes), stashes);

    return err;
}

_AMX afxError _AmxMdevCtorCb(afxPragmaDevice mdev, void** args, afxNat invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &mdev, afxFcc_MDEV);
    ...
    afxPragmaDeviceInfo const* info = ((afxPragmaDeviceInfo const *)args[1]) + invokeNo;
    AfxAssert(info);
    ...
    else
    {
        ...
        if (AfxAllocateInstanceData(mdev, AFX_COUNTOF(stashes), stashes)) AfxThrowError();
        else
        {
            if (!mdev->portCnt) AfxThrowError();
            else
            {
                AfxAssert(mdev->portCaps);

                for (afxNat i = 0; i < mdev->portCnt; i++)
                {
...
_AMX afxNat AfxInvokePragmaDevices(afxNat first, afxNat cnt, afxBool(*f)(afxPragmaDevice, void*), void *udd)
{
    afxError err = AFX_ERR_NONE;
    AfxAssert(cnt);
    AfxAssert(f);
    afxClass const* cls = AfxGetPragmaDeviceClass();
    AfxAssertClass(cls, afxFcc_MDEV);
    return AfxInvokeClassInstances(cls, first, cnt, (void*)f, udd);
}

_AMX afxNat AfxEvokePragmaDevices(afxBool(*flt)(afxPragmaDevice, void*), void* fdd, afxNat first, afxNat cnt, afxPragmaDevice devices[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssert(cnt);
    AfxAssert(devices);
...
_AMX afxNat AfxEnumeratePragmaDevices(afxNat first, afxNat cnt, afxPragmaDevice devices[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssert(cnt);
    AfxAssert(devices);
...
_AMX afxBool AfxGetPragmaDevice(afxNat mdevId, afxPragmaDevice* device)
{
    afxError err = AFX_ERR_NONE;
    afxPragmaDevice mdev = NIL;
    while (AfxEnumeratePragmaDevices(mdevId, 1, &mdev))
    {
        AfxAssertObjects(1, &mdev, afxFcc_MDEV);
        AfxAssert(device);
        *device = mdev;
...
_AMX afxError _AmxRegisterPragmaDevices(afxDriver icd, afxNat cnt, afxPragmaDeviceInfo const infos[], afxPragmaDevice devices[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &icd, afxFcc_ICD);
    AfxAssert(devices);

    afxClass* cls = (afxClass*)AfxGetPragmaDeviceClass();
    AfxAssertClass(cls, afxFcc_MDEV);
...
_AMX afxError afxScmHook(afxModule mdle, afxManifest const* ini)
{
    ...
    afxClassConfig const _AmxMdevClsCfg =
    {
        ...
        .fixedSiz = sizeof(AFX_OBJECT(afxPragmaDevice)),
