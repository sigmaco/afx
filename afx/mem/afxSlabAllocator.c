...
_AFXINL afxMemorySlab* _AfxFindSlabForPtr(afxSlabAllocator* mgr, void* p)
{
    ...
    afxNat range = mgr->unitSiz * mgr->unitsPerSlab;
...
_AFX void* AfxAllocateSlab(afxSlabAllocator* mgr)
{
    ...
    afxMemorySlab const* anchor = &mgr->anchor;
    afxMemorySlab* slab = anchor->next;
    afxNat unitsPerSlab = mgr->unitsPerSlab;
    ...
    if (slab)
    {
        ...
        ++slab->unitCnt;
        AfxAssert(unitsPerSlab >= slab->unitCnt);
...
_AFX afxError AfxDeallocateSlab(afxSlabAllocator* mgr, void* p)
{
    ...
    while (slab != anchor)
    {
        ...
        if (p2 >= base && p2 < &base[range])
        {
            afxMemorySlabSlot* slot = AfxRebase(p, afxMemorySlabSlot*, data);
...
_AFX afxError AfxSetUpSlabAllocator(afxSlabAllocator* mgr, afxNat unitSiz, afxNat unitsPerSlab)
{
    afxError err = NIL;
    AfxAssert(mgr);
    AfxAssert(unitSiz);
