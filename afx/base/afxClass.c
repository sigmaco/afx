...
_AFX afxError _AfxClsObjCtor(afxClass *cls, afxObject obj, void** args, afxNat invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertType(cls, afxFcc_CLS);
    afxClass* subset = AfxGetSubClass(cls);
...
_AFXINL afxError _AfxDummyObjCtor(afxObject obj, void** args, afxNat invokeNo)
{
    afxError err = AFX_ERR_NONE;
    (void)obj;
    (void)args;
...
_AFX afxError AfxRegisterClass(afxClass* cls, afxClass* subset, afxChain* host, afxClassConfig const* cfg)
{
    ...
    if (cls->fixedSiz) // only if dynamically allocated
    {
        AfxAssert(cls->fixedSiz > sizeof(afxObject)); // just to avoid non-open structure being passed in.
