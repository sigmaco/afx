...
_ASX afxError _AsxTraxStdCtorCb(afxWaveform trax, void** args, afxNat invokeNo)
{
    afxResult err = NIL;
    AfxAssertObjects(1, &trax, afxFcc_TRAX);
...
_ASX afxError AsxAcquireTrackers(afxSoundContext sctx, afxNat cnt, asxTracker trackers[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &sctx, afxFcc_SCTX);
    AfxAssert(trackers);
