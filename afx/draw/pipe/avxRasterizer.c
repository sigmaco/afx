...
_AVX void AfxDescribeRasterizerConfiguration(avxRasterizer razr, avxRasterizationConfig* config)
{
    ...
    if ((config->sampleCnt = razr->sampleCnt))
        AfxCopy2(config->sampleMasks, razr->sampleMasks, sizeof(razr->sampleMasks[0]), razr->sampleCnt);
    ...
    if ((config->colorOutCnt = razr->outCnt))
        AfxCopy2(config->colorOuts, razr->outs, sizeof(razr->outs[0]), razr->outCnt);

    AfxColorCopy(config->blendConstants, razr->blendConstants);
...
_AVX afxError _AvxRazrStdCtor(avxRasterizer razr, void** args, afxNat invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &razr, afxFcc_RAZR);
    ...
    AfxAssertObjects(1, &dctx, afxFcc_DCTX);
    avxRasterizationConfig const *rasc = ((avxRasterizationConfig const*)args[1]) + invokeNo;
