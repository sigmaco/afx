...
extern afxClassConfig const _AvxVtdClsCfg;
...
_AVX afxDrawDevice AfxGetDrawInputDevice(afxDrawInput din)
{
    ...
    afxDrawDevice ddev = AfxGetParent(din);
...
_AVX afxClass const* AfxGetMeshDataClass(afxDrawInput din)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &din, afxFcc_DIN);
    afxClass const* cls = &din->vtdCls;
    AfxAssertClass(cls, afxFcc_VTD);
    return cls;
}

_AVX afxClass const* AfxGetMeshTopologyClass(afxDrawInput din)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &din, afxFcc_DIN);
...
_AVX afxError _AvxDinCtorCb(afxDrawInput din, void** args, afxNat invokeNo)
{
    ...
    AfxRegisterClass(&din->mshtCls, NIL, classes, &tmpClsConf);
    
    tmpClsConf = _AvxVtdClsCfg;
    AfxRegisterClass(&din->vtdCls, NIL, classes, &tmpClsConf);
