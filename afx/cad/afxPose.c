...
_AMX afxNat AfxGetPoseCapacity(afxPose const lp)
{
...
_AMX afxTransform* AfxGetPoseTransform(afxPose const lp, afxNat artIdx)
{
    afxError err = AFX_ERR_NONE;
    AfxAssert(lp);
...
_AMX afxError _AmxPoseCtorCb(afxPose pose, void** args, afxNat invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &pose, afxFcc_POSE);
...
_AMX afxError AfxAcquirePoses(afxSimulation sim, afxNat cnt, afxNat const artCnt[], afxPose lp[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &sim, afxFcc_SIM);
