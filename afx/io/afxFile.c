...
_AFX afxBool _AfxIobEosFileCb(afxStream file)
{
    ...
    AfxAssert(file->typeFcc == afxFcc_FILE);
...
_AFX afxNat _AfxIobTellFileCb(afxStream file)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &file, afxFcc_IOB);
    AfxAssert(file->typeFcc == afxFcc_FILE);
...
_AFX afxError _AfxIobWriteFileCb(afxStream file, void const* const src, afxNat32 siz)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &file, afxFcc_IOB);
    AfxAssert(file->typeFcc == afxFcc_FILE);
...
_AFX afxError _AfxIobReadFileCb(afxStream file, void *dst, afxNat32 siz)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &file, afxFcc_IOB);
    AfxAssert(file->typeFcc == afxFcc_FILE);
...
_AFX afxError AfxReopenFile(afxStream iob, afxUri const* uri, afxFileFlags flags)
{
    ...
    if (AfxUriIsBlank(&dev)) AfxEnumerateStorages(0, 1, &disk);
...
_AFX afxError AfxReloadFile(afxStream iob, afxUri const* uri)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &iob, afxFcc_IOB);
    AfxAssert2(uri, !AfxUriIsBlank(uri));
