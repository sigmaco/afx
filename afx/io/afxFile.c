...
#if 0
_AFX afxError _AfxFileCtor(afxFile file, void** args, afxNat invokeNo)
{
    afxError err = AFX_ERR_NONE;
    
    afxFileFlags flags = *(afxFileFlags const*)(args[0]);
...
_AFX afxError AfxOpenFile(afxUri const* uri, afxIoFlags flags, afxStream* stream)
{
    afxError err = AFX_ERR_NONE;
    AfxAssert(uri);
    afxStream file = NIL;
    ...
    *modePtr++ = 'b';
    ...
    else
    {
        ...
        if (AfxAcquireImplementedStream(diskId, ioFlags, &fileStreamImpl, &idd, &file)) AfxThrowError();
        else
        {
...
_AFX afxError AfxLoadFile(afxUri const* uri, afxStream* stream)
{
    afxError err = AFX_ERR_NONE;
    afxStream iob = NIL;
    AfxAssert(uri);
    ...
    else
    {
        ...
        if (AfxAcquireStream(diskId, afxIoFlag_R, len, &iob)) AfxThrowError();
        else
        {
...
_AFX afxError AfxWrapFile(void* fd, afxIoFlags flags, afxStream* stream)
{
    afxError err = AFX_ERR_NONE;
    AfxAssert(fd);
    afxStream file = NIL;
