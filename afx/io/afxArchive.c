...
_AFX afxError AfxForkArchivedFile(afxArchive arc, afxNat idx, afxStream *ios)
{
    ...
    if (AfxAcquireStream(0, afxIoFlag_RWX, size, &ios2)) AfxThrowError();
    else
    {
        *ios = ios2;
...
_AFX afxError AfxOpenArchivedFile(afxArchive arc, afxNat idx, afxStream *in)
{
    ...
    if (AfxAcquireStream(0, afxIoFlag_RWX, size, &in2)) AfxThrowError();
    else
    {
        *in = in2;
...
_AFX afxError _AfxArcCtor(afxArchive arc, void** args, afxNat invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &arc, afxFcc_ARC);
...
_AFX afxArchive AfxOpenArchive(afxUri const* uri, afxFileFlags const flags, afxError* error)
{
    ...
    if (AfxOpenFile(uri, (flags & afxIoFlag_RWX), &file)) AfxThrowError();
    else
{
        afxStorage fsys = AfxGetParent(file);
