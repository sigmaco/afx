...
_AVX afxCmdId AvxCmdCommenceDrawScope(afxDrawContext dctx, avxDrawScope const* cfg)
{
    ...
    AFX_ASSERT(cmd);
    AfxCopyRect(&cmd->CommenceDrawScope.area, &cfg->area);
    cmd->CommenceDrawScope.baseLayer = cfg->baseLayer;
    cmd->CommenceDrawScope.layerCnt = cfg->layerCnt;
    cmd->CommenceDrawScope.targetCnt = cfg->targetCnt;
    cmd->CommenceDrawScope.canv = cfg->canv;
