...
_AVX afxUnit AfxGetDrawBridgePort(afxDrawBridge dexu)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dexu, afxFcc_DEXU);
    return dexu->portId;
...
_AVX void AfxQueryDrawQueues(afxDrawBridge dexu, afxUnit* baseQueIdx, afxUnit* queCnt)
{
    afxError err = AFX_ERR_NONE;
    /// dexu must be a valid afxDrawBridge handle.
    AfxAssertObjects(1, &dexu, afxFcc_DEXU);
    AFX_ASSERT(baseQueIdx || queCnt);
...
_AVX afxError _AvxSubmitDrawCommands(afxDrawBridge dexu, avxSubmission const* ctrl, afxUnit cnt, avxCmdb cmdbs[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dexu, afxFcc_DEXU);
    AFX_ASSERT(cnt);
    ...
    */
    ...
    AfxAssertObjects(1, &dctx, afxFcc_DCTX);
    afxUnit queCnt = AfxMin(dexu->queCnt, AfxMax(1, ctrl->queCnt));
...
_AVX afxError _AvxSubmitTransferences(afxDrawBridge dexu, avxTransference const* ctrl, afxUnit opCnt, void const* ops)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dexu, afxFcc_DEXU);
    AFX_ASSERT(opCnt);
...
_AVX afxError _AvxSubmitDrawWorkRequest(afxDrawBridge dexu, afxUnit cnt, avxQueueOp const subm[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dexu, afxFcc_DEXU);
    AFX_ASSERT(cnt);
...
_AVX afxError _AvxDexuStdCtorCb(afxDrawBridge dexu, void** args, afxUnit invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dexu, afxFcc_DEXU);
    ...
    afxDrawBridgeConfig const *cfg = ((afxDrawBridgeConfig const *)args[2]);
    AFX_ASSERT(cfg);
    afxUnit* baseQueIdxs = (afxUnit*)(args[3]);
    AFX_ASSERT(baseQueIdxs);

    if (!cfg) AfxThrowError();
    else
    {
        dexu->dctx = dctx;
        AFX_ASSERT_RANGE(AfxCountDrawPorts(ddev), cfg->portId, 1);
