...
#if 0
void LoadRLERow(afxRasterFile* tga, afxStream kIst, afxByte* dst, afxUnit uiDestSize)
{
...
_AVX afxError AfxWriteRasterFile(afxStream out, avxRasterIo* iop, afxUnit lodCnt, avxFormat fmt, avxRasterFlags flags, afxUnit uddSiz)
{
    ...
    */
    ...
    tgai.dataOff = iop->offset + sizeof(afxRasterFile) + uddSiz2;

    if (AfxWriteStreamAt(out, iop->offset, sizeof(afxRasterFile) + uddSiz2, 0, &tgai))
        AfxThrowError();
...
_AVX afxError AfxReadRasterFile(afxRasterFile* info, afxStream in)
{
    afxError err = NIL;
    afxUnit i;
...
_AVX afxError AfxDecodeRasterFile(afxRasterFile const* meta, afxStream in, void* dst)
{
    afxError err = NIL;
