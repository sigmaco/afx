...
_AVXINL avxClipSpaceDepth AfxGetCameraDepthRange(afxCamera cam)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_CAM, 1, &cam);
...
_AVXINL void AfxSetCameraDepthRange(afxCamera cam, avxClipSpaceDepth range)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_CAM, 1, &cam);
...
_AVXINL void AfxGetCameraOrientation2(afxCamera cam, afxReal* elevation, afxReal* azimuth, afxReal* roll)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_CAM, 1, &cam);
    AFX_ASSERT(elevation || azimuth || roll);
...
_AVXINL void AfxComputeCameraProjectiveMatrices(afxCamera cam, afxM4d p, afxM4d ip)
{
    ...
    else if (cam->depthRange != avxClipSpaceDepth_NEG_ONE_TO_ZERO)
    {
        if (far == 0.f)
        {
            p[2][2] = epsilon - 1.f;
...
_AVXINL void AfxComputeCameraProjectiveMatrices(afxCamera cam, afxM4d p, afxM4d ip)
{
...
#if 0
    avxClipSpaceDepth depthRange = cam->depthRange;

    if (depthRange == avxClipSpaceDepth_NEG_ONE_TO_ONE)
    {
        if (far == 0.f)
        {
            p[2][2] = epsilon - 1.f;
    ...
    else if (depthRange != avxClipSpaceDepth_NEG_ONE_TO_ZERO)
    {
        if (far == 0.f)
        {
            p[2][2] = epsilon - 1.f;
...
#if 0
_AVX afxBool AfxProcessCameraInteraction(afxCamera cam, afxUnit port, afxReal64 speed, afxReal64 dt)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_CAM, 1, &cam);
...
_AVX afxError _AvxCamCtorCb(afxCamera cam, void** args, afxUnit invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_CAM, 1, &cam);
    ...
    AfxGetClipSpaceInfo(din, &clip, &nonRh);
    AfxSetCameraDepthRange(cam, clip);
...
_AVX afxError AfxAcquireCameras(afxDrawInput din, afxUnit cnt, afxCamera cameras[])
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_DIN, 1, &din);

    afxClass* cls = (afxClass*)AvxGetCameraClass(din);
