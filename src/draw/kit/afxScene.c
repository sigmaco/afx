...
_AVX afxError AfxDrawSky(afxDrawContext dctx, afxScene scn)
{
    ...
    if (scn->skyType == avxSkyType_BOX)
    {
        ...
        AvxCmdBindRasters(dctx, 0, 1, 1, &scn->sky.cubemap, NIL);

        AvxCmdBindVertexSources(dctx, 0, 1, (afxBuffer[]) { scn->sky.cube }, NIL, NIL, (afxUnit32[]) { sizeof(afxV3d) });
...
_AVX afxError AfxAcquireScenes(afxDrawInput din, afxUnit cnt, afxSceneInfo infos[], afxScene scenes[])
{
    ...
    afxClass* cls = (afxClass*)AvxGetSceneClass(din);
