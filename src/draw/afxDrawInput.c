...
extern afxClassConfig const _AvxScnClsCfg;
...
_AVX afxClass const* AfxGetSceneClass(afxDrawInput din)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &din, afxFcc_DIN);
...
_AVX afxError AfxDisconnectDrawInput(afxDrawInput din)
{
    ...
    afxDrawContext dctx;
    AFX_ASSERT(!AfxGetDrawInputContext(din, &dctx));
    return err;
}
...
_AVX afxError _AvxDinCtorCb(afxDrawInput din, void** args, afxUnit invokeNo)
{
    ...
    tmpClsConf = _AvxScnClsCfg;
    AfxRegisterClass(&din->scnCls, NIL, classes, &tmpClsConf);
...
_AVX afxError AfxConfigureDrawInput(afxUnit ddevId, afxDrawInputConfig* cfg)
{
    afxError err = AFX_ERR_NONE;
...
_AVX afxError AfxOpenDrawInput(afxUnit ddevId, afxDrawInputConfig const* cfg, afxDrawInput* input)
{
    afxError err = AFX_ERR_NONE;
    afxDrawDevice ddev;
