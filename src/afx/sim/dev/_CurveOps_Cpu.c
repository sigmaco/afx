...
void(*Evaluators[4][10])(const afxReal *, const afxReal *, afxReal, afxReal *) =
{
  {
    NULL,
    SampleBSpline0x1,
...
void(*NormalizedEvaluators[4][10])(const afxReal *, const afxReal *, afxReal, afxReal *) =
{
  {
    NULL,
    SampleBSpline0x1,
...
{
    //((void(__cdecl *)(const afxReal *, const afxReal *, afxNat32, afxReal *))NormalizedEvaluators[Dimension-1][Degree+1])(ti, pi, t, Result);
    ((void(__cdecl *)(const afxReal *, const afxReal *, afxNat32, afxReal *))NormalizedEvaluators[0][Dimension + 10 * Degree])(ti, pi, t, Result);
}

void UncheckedSampleBSpline(int Degree, int Dimension, const afxReal *ti, const afxReal *pi, afxReal t, afxReal *Result)
{
    //((void(__cdecl *)(const afxReal *, const afxReal *, afxNat32, afxReal *))Evaluators[Dimension-1][Degree+1])(ti, pi, t, Result);
    ((void(__cdecl *)(const afxReal *, const afxReal *, afxNat32, afxReal *))Evaluators[0][Dimension + 10 * Degree])(ti, pi, t, Result);
}
...
void SampleBSpline(int Degree, int Dimension, afxBool Normalize, const afxReal *ti, const afxReal *pi, afxReal t, afxReal *Result)
{
    afxError err;
    int v7; // edx
    void(__cdecl *v8)(const afxReal *, const afxReal *, afxReal, afxReal *); // edx
    const char *Normalizea; // [esp+Ch] [ebp+Ch]
    const afxReal *pia; // [esp+14h] [ebp+14h]
    int ta; // [esp+18h] [ebp+18h]
    afxReal *Resulta; // [esp+1Ch] [ebp+1Ch]

    if (Degree < 0 || Degree >= 4)
    {
        Resulta = (afxReal *)4;
        pia = (const afxReal *)555;
    ...
    if (Dimension >= 0 && Dimension < 10)
    {
        ...
        if (v8)
        {
            ((void(__cdecl *)(const afxReal *, const afxReal *, afxNat32, afxReal *))v8)(ti, pi, t, Result);
            return;
        }
        Resulta = (afxReal *)Dimension;
        pia = (const afxReal *)571;
        Normalizea = "%s(%d): Unrecognized degree/dimension combination (%d/%d)";
        goto LABEL_13;
    }
    Resulta = (afxReal *)10;
    ta = Dimension;
    pia = (const afxReal *)556;
    Normalizea = "%s(%d): Dimension %d is out of range [0, %d)";
LABEL_14:
    AfxThrowError();
}

char ConstructBSplineBuffers(int Dimension, afxCurve PrevCurve, afxCurve Curve, afxCurve NextCurve, afxReal PrevCurveDuration, afxReal CurveDuration, afxReal NextCurveDuration, int KnotIndex, afxReal *ti, afxReal *pi, afxReal **tiPtr, afxReal **piPtr, const afxReal *IdentityVector)
{
    int v13; // edi
    int v14; // eax
    int v15; // esi
    ...
    afxReal TimeOffsetc; // [esp+18h] [ebp-10h]
    int KnotCount; // [esp+1Ch] [ebp-Ch]
    afxReal *tiDst; // [esp+20h] [ebp-8h]
    afxReal *tiDsta; // [esp+20h] [ebp-8h]
    int v61; // [esp+24h] [ebp-4h]
    afxReal *Curvea; // [esp+34h] [ebp+Ch]
    afxReal *Curveb; // [esp+34h] [ebp+Ch]
    afxReal *Curvec; // [esp+34h] [ebp+Ch]
    afxReal *Curved; // [esp+34h] [ebp+Ch]
    afxCurve Curvee; // [esp+34h] [ebp+Ch]
    afxReal Curvef; // [esp+34h] [ebp+Ch]
    ...
    if (!v17 && !v18)
    {
        v19 = (Curve->fmt == akxCurveFormat_DaK32fC32f) ? Curve : NIL;
    ...
    *piPtr = &pi[Dimension * v13];
    ...
    if (v17)
    {
        if (PrevCurve)
        {
            if (AfxCurveIsIdentity(PrevCurve))
            {
                v28 = ti;
                Curveb = pi;
                    ...
                    do
                    {
                        *v28 = v29;
                        ...
                        if (Dimension)
                        {
                            ...
                            do
                            {
                                *(afxReal *)((char *)v30 + (afxSize)Curveb - (char *)IdentityVector) = *v30;
                                ++v30;
                                --v31;
                            } while (v31);
                        }
            ...
            do
            {
                *v34 = ti[v32];
                ...
                if (Dimension)
                {
                    ...
                    do
                    {
                        *(afxReal *)((char *)v35 + (afxSize)Curvec - (char *)v33) = *v35;
                        ++v35;
                        --v36;
                    } while (v36);
                }
    ...
    if (AfxCurveIsIdentity(NextCurve))
    {
        v40 = ti[v32];
            ...
            do
            {
                ...
                *(v42 - 1) = v40;
                ...
                if (Dimension)
                {
                    ...
                    do
                    {
                        *(afxReal *)((char *)v43 + v44) = *v43;
                        ++v43;
                        --v45;
                    } while (v45);
                }

                v41 = (afxReal *)(((afxByte*)Curvee) + Dimension);
                --v21;
...
void EvaluateCurve(const struct sample_context *Context, afxCurve Curve, const afxReal *IdentityVector, int Dimension, afxReal *Result, afxBool CurveIsNormalized)
{
    ...
    if (AfxCurveIsIdentity(Curve))
    {
        for (afxNat i = Dimension; i-- > 0;)
            Result[i] = IdentityVector[i];
    }
    else if (AfxCurveIsConstantOrIdentity(Curve))
    {
        AfxExtractCurveKnotValue(Curve, 0, Result, IdentityVector);
    }
    else
    {
        afxNat Degree = AfxGetCurveDegree(Curve);
        afxNat v8 = AfxFindCurveKnot(Curve, Context->LocalClock);
...
int CurveKnotCharacter(afxCurve Curve)
{
    ...
    if (AfxCurveIsIdentity(Curve))
        ...
        result = 2 - (AfxCurveIsConstantOrIdentity(Curve) != 0);
...
_AKX smt2 Smt2 = 
{
    .UncheckedSampleBSplineN = UncheckedSampleBSplineN,
    .UncheckedSampleBSpline = UncheckedSampleBSpline,
    .CurveKnotCharacter = CurveKnotCharacter,
