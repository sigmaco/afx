...
_AVX afxError _AvxSampStdCtor(avxSampler samp, afxCookie const* cookie)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &samp, afxFcc_SAMP);

    avxYuvSamplerConfig const *cfg = ((avxYuvSamplerConfig const *)cookie->udd[1]) + cookie->no;
    afxBool yuv = cookie->udd[2] ? *(afxBool const*)cookie->udd[2] : FALSE;
    ...
    if (samp->yuv)
        ...
        samp->cfg = (avxYuvSamplerConfig) { 0 }, samp->cfg.base = cfg->base;
