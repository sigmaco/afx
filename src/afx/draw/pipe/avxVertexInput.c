...
_AVX afxError _AvxVinStdCtor(avxVertexInput vin, afxCookie const* cookie)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &vin, afxFcc_VIN);
    ...
    avxVertexInputStream const* streams = cookie->udd[2];
    afxNat attrCnt = *(afxNat const*)cookie->udd[3];
    avxVertexInputAttr const* attrs = cookie->udd[4];
