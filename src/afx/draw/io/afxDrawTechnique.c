...
_AVX afxError AfxUpdateRasterizationPass(afxDrawTechnique dtec, afxNat passIdx, avxRasterizationConfig const* cfg)
{
    ...
    afxDrawInput din = AfxGetObjectProvider(dtec);
    afxDrawContext dctx;
...
_AVX afxError _AvxDtecCtorCb(afxDrawTechnique dtec, afxCookie const* cookie)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dtec, afxFcc_DTEC);
    ...
    afxNat const passCnt = *(((afxNat const*)cookie->udd[1]) + cookie->no);
