...
_AVX afxError _AvxTxdStdCtor(avxTxd txd, afxCookie const* cookie)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &txd, afxFcc_TXD);

    avxTxdInfo const *txdi = ((avxTxdInfo const *)cookie->udd[1]) + cookie->no;

    afxDrawContext dctx = AfxGetObjectProvider(0);

    afxNat texCnt = txdi->texCnt;

    txd->texs = AfxAllocate(texCnt, sizeof(txd->texs[0]), 0, AfxHere());
    AfxAssert(txd->texs);
    AfxZero2(texCnt, sizeof(txd->texs[0]), txd->texs);
...
_AVX afxError AfxOpenTxd(afxDrawInput din, afxUri const* uri, avxTxd* dict)
{
    ...
    afxStream iob = AfxOpenFile(uri, afxIoFlag_RX);

    if (!iob) AfxThrowError();
    else
    {
        afxNat32 type;
        afxNat32 len;
