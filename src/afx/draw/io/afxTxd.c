...
afxBool _rwD3D9NativeTextureRead(afxStream stream)
{
    afxError err = NIL;
    afxRaster raster = NIL;
    afxNat32            length, version;
    ...
    if (AfxReadStream(stream, sizeof(_rwD3D9NativeTexture), 0, (void *)&nativeTexture))
    {
        AfxThrowError();
    }
    ...
    if (AfxReadStream(stream, sizeof(_rwD3D9NativeRaster), 0, (void *)&nativeRaster))
    {
        AfxThrowError();
    }
    ...
    do
    {
        for (afxNat i = 0; i < info.lodCnt; i++)
        {
            ...
            // Read the mip level
            AfxUploadRaster(raster, &iop, 1, stream);
        }
    } while (--face);
...
RwTexDictionary * RwTexDictionaryStreamRead(afxStream *stream)
{
    afxNat32            size, version;
