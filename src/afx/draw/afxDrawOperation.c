...
_AVXINL afxError AfxDrawOperationBlueprintAddPass(afxDrawOperationBlueprint *blueprint, afxNat tecIdx, afxNat *passIdx, afxString const *name)
{
    ...
    else
    {
        ...
        pass->hasIa = FALSE;
        pass->hasRs = FALSE;
        pass->inputAssembly = (avxPipelinePrimitiveState) { 0 };
        pass->rasterization = (avxPipelineRasterizerState) { 0 };
        pass->depthHandling = (avxPipelineDepthState) { 0 };
...
_AVXINL void AfxDrawOperationBlueprintConfigRasterizerState(afxDrawOperationBlueprint *blueprint, afxNat tecIdx, afxNat passIdx, avxPipelineRasterizerState const *state)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertType(blueprint, afxFcc_DOPB);
    AfxAssert(tecIdx < AfxCountArrayElements(&blueprint->techniques));
    ...
    AfxAssert(passIdx < AfxCountArrayElements(&tec->passes));
    afxDrawOperationBlueprintTechniquePass *pass = AfxGetArrayUnit(&tec->passes, passIdx);
    pass->rasterization = state ? *state : (avxPipelineRasterizerState) { 0 };
    pass->hasRs = !!state;
}

_AVXINL void AfxDrawOperationBlueprintConfigDepthState(afxDrawOperationBlueprint *blueprint, afxNat tecIdx, afxNat passIdx, avxPipelineDepthState const *state)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertType(blueprint, afxFcc_DOPB);
    AfxAssert(tecIdx < AfxCountArrayElements(&blueprint->techniques));
    ...
    AfxAssert(passIdx < AfxCountArrayElements(&tec->passes));
    afxDrawOperationBlueprintTechniquePass *pass = AfxGetArrayUnit(&tec->passes, passIdx);
    pass->depthHandling = state ? *state : (avxPipelineDepthState) { 0 };
    pass->hasDs = !!state;
}

_AVXINL void AfxDrawOperationBlueprintConfigInputAssemblyState(afxDrawOperationBlueprint *blueprint, afxNat tecIdx, afxNat passIdx, avxPipelinePrimitiveState const *state)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertType(blueprint, afxFcc_DOPB);
    AfxAssert(tecIdx < AfxCountArrayElements(&blueprint->techniques));
    ...
    AfxAssert(passIdx < AfxCountArrayElements(&tec->passes));
    afxDrawOperationBlueprintTechniquePass *pass = AfxGetArrayUnit(&tec->passes, passIdx);
    pass->inputAssembly = state ? *state : (avxPipelinePrimitiveState) { 0 };
    pass->hasIa = !!state;
}
...
_AVX avxPipeline AfxDrawOperationGetPipeline(afxDrawOperation dop, afxNat tecIdx, afxNat passIdx)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObject(dop, afxFcc_DOP);
    AfxAssert(dop->techCnt > tecIdx);
    AfxAssert(dop->techniques[tecIdx].passCnt > passIdx);
    avxPipeline pip = dop->techniques[tecIdx].passes[passIdx].pip;
