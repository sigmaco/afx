...
_AFX afxManager* AfxGetFenceClass(afxContext ctx)
{
    afxError err = AFX_ERR_NONE;
    //AfxAssertObjects(1, &ddev, afxFcc_CTX);
    afxManager *cls = &ctx->fences;
    AfxAssertClass(cls, afxFcc_FENC);
    return cls;
}

_AFX afxManager* AfxGetSemaphoreClass(afxContext ctx)
{
    afxError err = AFX_ERR_NONE;
    //AfxAssertObjects(1, &ddev, afxFcc_DEV);
    afxManager *cls = &ctx->semaphores;
