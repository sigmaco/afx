...
_AFX afxContext AfxGetFenceContext(afxFence fenc)
{
    ...
    afxContext ctx = AfxGetObjectProvider(fenc);
...
_AFX afxDevice AfxGetFenceDevice(afxFence fenc)
{
    ...
    AfxAssert(ctx);
    afxDevice dev = AfxGetObjectProvider(ctx);
    //AfxAssertObjects(1, &ctx, afxFcc_CTX);
    AfxAssert(dev);
    return dev;
}
...
_AFX afxError _AfxFencStdCtor(afxFence fenc, afxCookie const* cookie)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &fenc, afxFcc_FENC);

    afxContext ctx = cookie->udd[0];
...
_AFX afxError AfxAcquireFences(afxContext ctx, afxBool signaled, afxNat cnt, afxFence fences[])
{
    afxError err = AFX_ERR_NONE;
    //AfxAssertObjects(1, &ctx, afxFcc_DCTX);
    AfxAssert(cnt);
    ...
    afxManager* cls = AfxGetFenceClass(ctx);
