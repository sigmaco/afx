...
_AMX afxError AmxMapBuffers(afxMixSystem msys, afxUnit cnt, amxBufferedMap maps[], void** placeholders[])
{
    ...
    if (_AmxMsysGetDdi(msys)->remapCb(msys, FALSE, opCnt, remaps2))
    {
        AfxThrowError();
    }
...
_AMX afxError AmxUnmapBuffers(afxMixSystem msys, afxUnit cnt, amxBufferedMap maps[])
{
    ...
    */
    ...
    if (_AmxMsysGetDdi(msys)->remapCb(msys, TRUE, cnt, unmaps2))
    {
        AfxThrowError();
    }
...
_AMX afxError AmxCohereMappedBuffers(afxMixSystem msys, afxBool discard, afxUnit cnt, amxBufferedMap const maps[])
{
    ...
    if (_AmxMsysGetDdi(msys)->cohereCb(msys, discard, cnt, maps))
    {
...
_AMX afxError AmxCopyBuffers(afxMixSystem msys, afxMask exuMask, afxUnit cnt, amxBufferedCopy const ops[])
{
    afxError err = { 0 };
    // @msys must be a valid afxMixSystem handle.
    AFX_ASSERT_OBJECTS(afxFcc_MSYS, 1, &msys);
    ...
    for (afxUnit i = 0; i < cnt; i++)
    {
        amxBufferedCopy const* op = &ops[i];
        AFX_ASSERT_ALIGNMENT(op->dstOffset, AMX_BUFFER_ALIGNMENT);
        AFX_ASSERT_ALIGNMENT(op->srcOffset, AMX_BUFFER_ALIGNMENT);

        amxBuffer dst = op->dst;
        AFX_ASSERT_OBJECTS(afxFcc_MBUF, 1, &dst);
        AFX_ASSERT_RANGE(AmxGetBufferCapacity(dst, 0), op->dstOffset, op->range);

        amxBuffer src = op->src;
        AFX_ASSERT_OBJECTS(afxFcc_MBUF, 1, &src);
        AFX_ASSERT_RANGE(AmxGetBufferCapacity(src, 0), op->srcOffset, op->range);
        ...
        // Perform the data transfer (buffer update)
        if (_AmxMsysGetDdi(msys)->transferCb(msys, &transfer, 1, &op2))
        {
            // If transfer fails, throw an error
            AfxThrowError();
            //return err;
