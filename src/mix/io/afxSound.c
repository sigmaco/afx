...
_AMX void AmxTransformSound(afxSound snd, afxTransform const* t)
{
...
_AMX afxUnit AmxAccumulateSoundSamples(afxSound snd, afxUnit sampleCnt, afxUnit chCnt, afxUnit freq, afxUnit bufCap, afxReal output[])
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_SND, 1, &snd);
...
_AMX afxError AmxRewindSound(afxSound snd)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_SND, 1, &snd);
...
_AMX afxBool AmxHasSoundEnded(afxSound snd)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_SND, 1, &snd);
...
_AMX afxError AfxDoSound(afxMixSystem msys, afxSoundInfo const* info, afxUnit* soundId)
{
    afxResult err = NIL;
    AFX_ASSERT_OBJECTS(afxFcc_MSYS, 1, &msys);
    AFX_ASSERT(soundId);
