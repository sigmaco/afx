        ...
        if (msh) // attach operation
        {
            ...
            if (!err && mshi.mtlCnt)
            {
                ...
                else
                {
                    ...
                    if (mshi.mtlCnt != (mappedMtlCnt = ArxFindMaterialIndices(AfxGetProvider(msh), mshi.mtlCnt, materials, mtlToTxdMap)))
                    {
...
_ARX afxError _ArxMdlCtorCb(arxModel mdl, void** args, afxUnit invokeNo)
{
    ...
    if (!AfxCatalogStrings(_ArxMorpGetModelUrnStringBase(morp), 1, &mdlb->urn.s, &mdl->urn))
    {
        AfxThrowError();
        return err;
    }
    ...
    else
    {
        ...
        else if (!AfxCatalogStrings(_ArxMorpGetPivotTagStringBase(morp), jntCnt, mdlb->joints, mdl->jntId))
            AfxThrowError();
...
_ARX afxUnit ArxEnumerateModels(arxGeome morp, afxUnit first, afxUnit cnt, arxModel models[])
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_MORP, 1, &morp);
    afxClass const* cls = _ArxMorpGetModelClass(morp);
    AFX_ASSERT_CLASS(cls, afxFcc_MDL);
    return AfxEnumerateObjects(cls, first, cnt, (afxObject*)models);
}

_ARX afxError ArxAssembleModels(arxGeome morp, afxUnit cnt, arxModelBlueprint const blueprints[], arxModel models[])
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_MORP, 1, &morp);
    ...
    afxClass* cls = (afxClass*)_ArxMorpGetModelClass(morp);
    AFX_ASSERT_CLASS(cls, afxFcc_MDL);
