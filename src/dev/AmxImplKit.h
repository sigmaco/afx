...
AFX_DEFINE_STRUCT(afxComboDeviceInfo)
{
    ...
    afxError(*openSimCb)(afxComboDevice, afxSimulation, void** udd, afxUnit invokeNo); // unused if a custom dctx ctor is installed.
...
#endif
{
    ...
    afxError(*waitCb)(afxComboBridge, afxUnit);
    afxUnit(*submitCb)(afxComboBridge, amxSubmission const*, afxUnit, avxCmdb[]);
    afxUnit(*submCb)(afxComboBridge mdge, afxFence fenc, afxUnit cnt, amxQueueOp const req[]);
};
#endif//_AMX_MATH_BRIDGE_C

AMX afxReal _AkxGetAllowedLodErrorFadingFactor(afxSimulation sim);
AMX afxError _AmxRegisterComboDevices(afxModule icd, afxUnit cnt, afxComboDeviceInfo const infos[], afxComboDevice devices[]);

AMX afxClass const* _AmxGetMathQueueClass(afxComboBridge mdge);
AMX afxUnit _AmxCountMathQueues(afxComboBridge mdge, afxUnit baseQueIdx);
AMX afxBool _AmxGetMathQueue(afxComboBridge mdge, afxUnit queIdx, afxMathQueue* queue);
AMX afxError _AmxWaitForIdleMathQueue(afxComboBridge mdge, afxUnit queIdx);
