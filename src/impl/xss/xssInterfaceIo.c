...
void _XsFsysMount(xssVm vm)
{
    ...
    afxError err = AfxMountStorageUnit(point.str.start[0], &endpoint, ioFlags);
...
void _XsFsysDismount(xssVm vm)
{
    ...
    afxError err = AfxDismountStorageUnit(point.str.start[0], &endpoint, ioFlags);
    XssPushError(vm, 0, err);
}
...
void _XsCamApplyOrientation(xssVm vm)
{
    ...
    afxV3d v;
    XssPullReal3(vm, 1, v);
    AfxApplyCameraOrientation(cam, v);
}
