...
static uint32_t validateIndexValue(xssVm vm, uint32_t count, double value,
                                   const char* argName)
{
  ...
  vm->fiber->error = xssStringFormat(vm, "$ out of bounds.", argName);
  return UINT32_MAX;
}
...
{
  if (xssMapIsValidKey(arg)) return true;
