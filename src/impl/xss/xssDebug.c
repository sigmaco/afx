...
void xssDebugPrintStackTrace(xssVm env)
{
  ...
  if (env->config.errorFn == NULL) return;
  ...
  for (int i = fiber->numFrames - 1; i >= 0; i--)
  {
    ...
    // detail of what part of the core module is written in C and what is Luna.
...
static int dumpInstruction(xssVm env, ObjFn* fn, int i, int* lastLine)
{
  int start = i;
  ...
  switch (code)
  {
    case CODE_CONSTANT:
    {
      ...
      xssDumpValue(fn->constants.data[constant]);
      printf("'\n");
      break;
    }
    ...
    case CODE_CALL_16:
    {
      ...
      printf("CALL_%-11d %5d '%s'\n", numArgs, symbol,
             env->methodNames.data[symbol]->value);
      break;
    }
    ...
    case CODE_SUPER_16:
    {
      ...
      printf("SUPER_%-10d %5d '%s' %5d\n", numArgs, symbol,
             env->methodNames.data[symbol]->value, superclass);
      break;
    }
    ...
    case CODE_IMPORT_MODULE:
    {
      ...
      xssDumpValue(fn->constants.data[name]);
      printf("'\n");
      break;
    }
    ...
    case CODE_IMPORT_VARIABLE:
    {
      ...
      xssDumpValue(fn->constants.data[variable]);
      printf("'\n");
      break;
    }
...
{
  return dumpInstruction(env, fn, i, NULL);
}
...
void xssDumpCode(xssVm env, ObjFn* fn)
{
  ...
  for (;;)
  {
    int offset = dumpInstruction(env, fn, i, &lastLine);
    if (offset == -1) break;
    i += offset;
  }
