...
DEF_PRIMITIVE(fiber_new)
{
  ...
  RETURN_OBJ(xssNewFiber(vm, closure));
}
...
static bool runFiber(xssVm vm, ObjFiber* fiber, Value* args, bool isCall,
                     bool hasValue, const char* verb)
{

  if (xssHasError(fiber))
  {
...
DEF_PRIMITIVE(fiber_isDone)
{
  ...
  RETURN_BOOL(runFiber->numFrames == 0 || xssHasError(runFiber));
}
...
DEF_PRIMITIVE(fiber_try)
{
  ...
  if (!xssHasError(vm->fiber)) vm->fiber->state = FIBER_TRY;
  return false;
}
...
static void call_fn(xssVm vm, Value* args, int numArgs)
{
  // +1 to include the function itself.
  xssCallFunction(vm, vm->fiber, AS_CLOSURE(args[0]), numArgs + 1);
}
...
DEF_PRIMITIVE(list_filled)
{
  ...
  ObjList* list = xssNewList(vm, size);
...
{
  RETURN_OBJ(xssNewList(vm, 0));
}

DEF_PRIMITIVE(list_add)
{
  xssValueBufferWrite(vm, &AS_LIST(args[0])->elements, args[1]);
  RETURN_VAL(args[1]);
}
...
{
  xssValueBufferClear(vm, &AS_LIST(args[0])->elements);
  RETURN_NULL;
}
...
DEF_PRIMITIVE(list_insert)
{
  ...
  xssListInsert(vm, list, args[2], index);
  RETURN_VAL(args[2]);
}
...
DEF_PRIMITIVE(list_removeAt)
{
  ...
  RETURN_VAL(xssListRemoveAt(vm, list, index));
}

DEF_PRIMITIVE(list_removeValue) {
  ObjList* list = AS_LIST(args[0]);
  int index = xssListIndexOf(vm, list, args[1]);
...
{
  ObjList* list = AS_LIST(args[0]);
  RETURN_NUM(xssListIndexOf(vm, list, args[1]));
}
...
DEF_PRIMITIVE(list_subscript)
{
  ...
  ObjList* result = xssNewList(vm, count);
...
DEF_PRIMITIVE(map_subscript)
{
  ...
  Value value = xssMapGet(map, args[1]);
...
DEF_PRIMITIVE(map_subscriptSetter)
{
  ...
  xssMapSet(vm, AS_MAP(args[0]), args[1], args[2]);
  RETURN_VAL(args[2]);
}
...
DEF_PRIMITIVE(map_containsKey)
{
  ...
  RETURN_BOOL(!IS_UNDEFINED(xssMapGet(AS_MAP(args[0]), args[1])));
}
...
DEF_PRIMITIVE(map_remove)
{
  ...
  RETURN_VAL(xssMapRemoveKey(vm, AS_MAP(args[0]), args[1]));
}
...
DEF_NUM_CONSTANT(nan,      LUNA_DOUBLE_NAN)
...
DEF_PRIMITIVE(num_dotDot)
{
  ...
  RETURN_VAL(xssNewRange(vm, from, to, true));
}
...
DEF_PRIMITIVE(num_dotDotDot)
{
  ...
  double from = AS_NUM(args[0]);
  double to = AS_NUM(args[1]);
  RETURN_VAL(xssNewRange(vm, from, to, false));
}
...
{
  RETURN_VAL(xssNumToString(vm, AS_NUM(args[0])));
}
...
{
    (void)vm;
  RETURN_BOOL(xssValuesEqual(args[1], args[2]));
}
...
{
    (void)vm;
  RETURN_BOOL(xssValuesEqual(args[0], args[1]));
}
...
DEF_PRIMITIVE(object_is)
{
  ...
  ObjClass *classObj = xssGetClass(vm, args[0]);
...
DEF_PRIMITIVE(object_toString)
{
  ...
  RETURN_VAL(xssStringFormat(vm, "instance of @", name));
}

DEF_PRIMITIVE(object_type)
{
    (void)vm;
  RETURN_OBJ(xssGetClass(vm, args[0]));
}
...
DEF_PRIMITIVE(range_toString)
{
  ...
  Value from = xssNumToString(vm, range->from);
  xssPushRoot(vm, AS_OBJ(from));

  Value to = xssNumToString(vm, range->to);
  xssPushRoot(vm, AS_OBJ(to));

  Value result = xssStringFormat(vm, "@$@", from,
...
DEF_PRIMITIVE(string_fromCodePoint)
{
  ...
  RETURN_VAL(xssStringFromCodePoint(vm, codePoint));
}
...
DEF_PRIMITIVE(string_fromByte)
{
  ...
  RETURN_VAL(xssStringFromByte(vm, (uint8_t) byte));
}
...
DEF_PRIMITIVE(string_codePointAt)
{
  ...
  RETURN_NUM(xssUtf8Decode((uint8_t*)string->value + index,
                            string->length - index));
}
...
DEF_PRIMITIVE(string_contains)
{
  ...
  RETURN_BOOL(xssStringFind(string, search, 0) != UINT32_MAX);
}
...
DEF_PRIMITIVE(string_indexOf1)
{
  ...
  uint32_t index = xssStringFind(string, search, 0);
...
DEF_PRIMITIVE(string_indexOf2)
{
  ...
  uint32_t index = xssStringFind(string, search, start);
  RETURN_NUM(index == UINT32_MAX ? -1 : (int)index);
}
...
DEF_PRIMITIVE(string_iteratorValue)
{
  ...
  RETURN_VAL(xssStringCodePointAt(vm, string, index));
}
...
DEF_PRIMITIVE(string_plus)
{
  ...
  RETURN_VAL(xssStringFormat(vm, "@@", args[0], args[1]));
}
...
DEF_PRIMITIVE(string_subscript)
{
  ...
  RETURN_VAL(xssNewStringFromRange(vm, string, start, count, step));
}
...
static ObjClass* defineClass(xssVm vm, ObjModule* module, const char* name)
{
  ObjString* nameString = AS_STRING(xssNewString(vm, name));
  xssPushRoot(vm, (Obj*)nameString);

  ObjClass* classObj = xssNewSingleClass(vm, 0, nameString);

  xssDefineVariable(vm, module, name, nameString->length, OBJ_VAL(classObj), NULL);

  xssPopRoot(vm);
  return classObj;
}

void xssInitializeCore(xssVm vm)
{
  ObjModule* coreModule = xssNewModule(vm, NULL);
  xssPushRoot(vm, (Obj*)coreModule);
  
  // The core module's key is null in the module map.
  xssMapSet(vm, vm->modules, NULL_VAL, OBJ_VAL(coreModule));
  xssPopRoot(vm); // coreModule.
  ...
  xssBindSuperclass(vm, vm->classClass, vm->objectClass);
  ...
  // collected.
  xssBindSuperclass(vm, objectMetaclass, vm->classClass);
  ...
  xssInterpret(vm, NULL, coreModuleSource);

  vm->boolClass = AS_CLASS(xssFindVariable(vm, coreModule, "Bool"));
  PRIMITIVE(vm->boolClass, "toString", bool_toString);
  PRIMITIVE(vm->boolClass, "!", bool_not);

  vm->fiberClass = AS_CLASS(xssFindVariable(vm, coreModule, "Fiber"));
  ...
  vm->fnClass = AS_CLASS(xssFindVariable(vm, coreModule, "Fn"));
  ...
  vm->nullClass = AS_CLASS(xssFindVariable(vm, coreModule, "Null"));
  PRIMITIVE(vm->nullClass, "!", null_not);
  PRIMITIVE(vm->nullClass, "toString", null_toString);

  vm->numClass = AS_CLASS(xssFindVariable(vm, coreModule, "Num"));
  ...
  vm->stringClass = AS_CLASS(xssFindVariable(vm, coreModule, "String"));
  ...
  vm->listClass = AS_CLASS(xssFindVariable(vm, coreModule, "List"));
  ...
  vm->mapClass = AS_CLASS(xssFindVariable(vm, coreModule, "Map"));
  ...
  vm->rangeClass = AS_CLASS(xssFindVariable(vm, coreModule, "Range"));
  ...
  ObjClass* systemClass = AS_CLASS(xssFindVariable(vm, coreModule, "System"));
