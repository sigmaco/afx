...
_AUX afxError AfxCloseEnvironment(void)
{
    ...
    if (AfxGetEnvironment(&env))
    {
        ...
        AfxExhaustChainedClasses(&env->classes);
...
_AUX afxError AfxOpenEnvironment(afxEnvironment env, afxUri const* host, afxAuthMethod method, afxString const* credential)
{
    ...
    afxEnvironment curr;
    if (AfxGetEnvironment(&curr))
    {
        AFX_ASSERT_OBJECTS(afxFcc_ENV, 1, &curr);
        AfxCloseEnvironment();
    }
...
_AUX afxError _AuxEnvCtorCb(afxEnvironment env, void** args, afxUnit invokeNo)
{
...

    {
        ...
        AfxMountClass(&env->wndCls, NIL, &env->classes, &clsCfg);

        clsCfg = /*cfg->fntClsCfg ? *cfg->fntClsCfg :*/ _AUX_FNT_CLASS_CONFIG;
        AfxMountClass(&env->fntCls, NIL, &env->classes, &clsCfg);

        clsCfg = /*cfg->themClsCfg ? *cfg->themClsCfg :*/ _AUX_THEM_CLASS_CONFIG;
        AfxMountClass(&env->themCls, NIL, &env->classes, &clsCfg);

        clsCfg = cfg->xssClsCfg ? *cfg->xssClsCfg : (afxClassConfig) { 0 };
        AfxMountClass(&env->xssCls, NIL, &env->classes, &clsCfg);
    }

    AfxMakeString4096(&env->clipb, NIL);
