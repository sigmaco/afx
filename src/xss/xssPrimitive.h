#ifndef xssPrimitive_h
#define xssPrimitive_h

#include "xssVm.h"

// Binds a primitive method named [name] (in Luna) implemented using C function
// [fn] to `ObjClass` [cls].
#define PRIMITIVE(cls, name, function)                                         \
    do                                                                         \
    {                                                                          \
      int symbol = xssSymbolTableEnsure(vm,                                   \
          &vm->methodNames, name, strlen(name));                               \
      Method method;                                                           \
      method.type = METHOD_PRIMITIVE;                                          \
      method.as.primitive = prim_##function;                                   \
      xssBindMethod(vm, cls, symbol, method);                                 \
    } while (false)
...
#define DEF_PRIMITIVE(name)                                                    \
    static bool prim_##name(xssVm vm, Value* args)
...
#define RETURN_ERROR(msg)                                                      \
    do                                                                         \
    {                                                                          \
      vm->fiber->error = xssNewStringLength(vm, msg, sizeof(msg) - 1);        \
      return false;                                                            \
    } while (false)
...
// `UINT32_MAX`.
uint32_t validateIndex(xssVm vm, Value arg, uint32_t count,
                       const char* argName);

// Validates that the given [arg] is a String. Returns true if it is. If not,
// reports an error and returns false.
bool validateString(xssVm vm, Value arg, const char* argName);
...
// range is decreasing.
uint32_t calculateRange(xssVm vm, ObjRange* range, uint32_t* length,
                        int* step);
