...
// Since Luna is dynamically typed, any variable can hold a value of any type,
...
// storage location that can hold any Luna value. The stack, module variables,
...
// Luna implementation calls these "Obj", or objects, though to a user, all
// values are objects.
//
...
//
// The representation is controlled by the `LUNA_NAN_TAGGING` define. If that's
...
#define AS_NUM(value)       (xssValueToNum(value))             // double
...
// These macros promote a primitive C value to a full Luna Value. There are
...
#define NUM_VAL(num) (xssNumToValue(num))                      // double
#define OBJ_VAL(obj) (xssObjectToValue((Obj*)(obj)))           // Any Obj___*

// These perform type tests on a Value, returning `true` if the Value is of the
// given type.
#define IS_BOOL(value) (xssIsBool(value))                      // Bool
#define IS_CLASS(value) (xssIsObjType(value, OBJ_CLASS))       // ObjClass
#define IS_CLOSURE(value) (xssIsObjType(value, OBJ_CLOSURE))   // ObjClosure
#define IS_FIBER(value) (xssIsObjType(value, OBJ_FIBER))       // ObjFiber
#define IS_FN(value) (xssIsObjType(value, OBJ_FN))             // ObjFn
#define IS_FOREIGN(value) (xssIsObjType(value, OBJ_FOREIGN))   // ObjForeign
#define IS_INSTANCE(value) (xssIsObjType(value, OBJ_INSTANCE)) // ObjInstance
#define IS_LIST(value) (xssIsObjType(value, OBJ_LIST))         // ObjList
#define IS_MAP(value) (xssIsObjType(value, OBJ_MAP))           // ObjMap
#define IS_RANGE(value) (xssIsObjType(value, OBJ_RANGE))       // ObjRange
#define IS_STRING(value) (xssIsObjType(value, OBJ_STRING))     // ObjString
...
#define CONST_STRING(env, text) xssNewStringLength((env), (text), sizeof(text) - 1)
...
typedef struct sObjUpvalue
{
  ...
  // collected, but they are not first class Luna objects.
  Obj obj;
...
typedef bool (*Primitive)(xssVm env, Value* args);
...
typedef enum
{
  ...
  // initial fiber used by a call to `xssCall()` or `xssInterpret()`.
  FIBER_ROOT,
...
// [stackStart].
static inline void xssAppendCallFrame(xssVm env, ObjFiber* fiber,
...
static inline bool xssHasError(const ObjFiber* fiber)
{
...
static inline bool xssMapIsValidKey(Value arg);
...
// [needle].
uint32_t xssStringFind(ObjString* haystack, ObjString* needle,
                        uint32_t startIndex);

// Returns true if [a] and [b] represent the same string.
static inline bool xssStringEqualsCString(const ObjString* a,
...
//
// Unlike xssGetClassInline in xssVm.h, this is not inlined. Inlining helps
...
static inline bool xssValuesSame(Value a, Value b)
{
#if LUNA_NAN_TAGGING
...
// [IS_BOOL].
static inline bool xssIsBool(Value value)
{
#if LUNA_NAN_TAGGING
...
static inline bool xssIsObjType(Value value, ObjType type)
{
  return IS_OBJ(value) && AS_OBJ(value)->type == type;
}

// Converts the raw object pointer [obj] to a [Value].
static inline Value xssObjectToValue(Obj* obj)
{
#if LUNA_NAN_TAGGING
...
static inline double xssValueToNum(Value value)
{
#if LUNA_NAN_TAGGING
  return xssDoubleFromBits(value);
#else
  return value.as.num;
#endif
}
...
static inline Value xssNumToValue(double num)
{
#if LUNA_NAN_TAGGING
  return xssDoubleToBits(num);
#else
  Value value;
  value.type = VAL_NUM;
  value.as.num = num;
