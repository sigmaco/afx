...
#if LUNA_DEBUG_DUMP_COMPILED_CODE
  #include "xssDebug.h"
#endif
                       ...
                       const char* format, va_list args)
{
  ...
  // Only report errors if there is a LunaErrorFn to handle them.
  if (parser->env->config.errorFn == NULL) return;

  // Format the label and message.
  char message[ERROR_MESSAGE_SIZE];
  int length = AfxSprintf(message, "%s: ", label);
  length += AfxVsprintf(message + length, format, args);
  ...
  parser->env->config.errorFn(parser->env, LUNA_ERROR_COMPILE,
...
static int addConstant(Compiler* compiler, Value constant)
{
  ...
  {
    Value existing = xssMapGet(compiler->constants, constant);
  ...
  {
    if (IS_OBJ(constant)) xssPushRoot(compiler->parser->env, AS_OBJ(constant));
    xssValueBufferWrite(compiler->parser->env, &compiler->fn->constants,
                         constant);
    if (IS_OBJ(constant)) xssPopRoot(compiler->parser->env);
    
    if (compiler->constants == NULL)
    {
      compiler->constants = xssNewMap(compiler->parser->env);
    }
    xssMapSet(compiler->parser->env, compiler->constants, constant,
                         ...
                         bool isMethod)
{
  ...
  compiler->attributes = xssNewMap(parser->env);
  compiler->fn = xssNewFunction(parser->env, parser->module,
                                 compiler->numLocals);
}
...
{
  ByteBuffer string;
  xssByteBufferInit(&string);
  xssByteBufferWrite(parser->env, &string, firstChar);

  while (isName(peekChar(parser)) || isDigit(peekChar(parser)))
  {
    char c = nextChar(parser);
    xssByteBufferWrite(parser->env, &string, c);
  }
  ...
  parser->next.value = xssNewStringLength(parser->env,
                                            (char*)string.data, string.count);

  xssByteBufferClear(parser->env, &string);
...
static void readUnicodeEscape(Parser* parser, ByteBuffer* string, int length)
{
  ...
  int numBytes = xssUtf8EncodeNumBytes(value);
  if (numBytes != 0)
  {
    xssByteBufferFill(parser->env, string, 0, numBytes);
    xssUtf8Encode(value, string->data + string->count - numBytes);
  }
}
...
static void readString(Parser* parser)
{
  ...
  for (;;)
  {
    ...
    if (c == '\\')
    {
      ...
      {
        case '"':  xssByteBufferWrite(parser->env, &string, '"'); break;
        case '\\': xssByteBufferWrite(parser->env, &string, '\\'); break;
        case '%':  xssByteBufferWrite(parser->env, &string, '%'); break;
        case '0':  xssByteBufferWrite(parser->env, &string, '\0'); break;
        case 'a':  xssByteBufferWrite(parser->env, &string, '\a'); break;
        case 'b':  xssByteBufferWrite(parser->env, &string, '\b'); break;
        case 'e':  xssByteBufferWrite(parser->env, &string, '\33'); break;
        case 'f':  xssByteBufferWrite(parser->env, &string, '\f'); break;
        case 'n':  xssByteBufferWrite(parser->env, &string, '\n'); break;
        case 'r':  xssByteBufferWrite(parser->env, &string, '\r'); break;
        case 't':  xssByteBufferWrite(parser->env, &string, '\t'); break;
        case 'u':  readUnicodeEscape(parser, &string, 4); break;
        case 'U':  readUnicodeEscape(parser, &string, 8); break;
        case 'v':  xssByteBufferWrite(parser->env, &string, '\v'); break;
        case 'x':
          xssByteBufferWrite(parser->env, &string,
...
static void nextToken(Parser* parser)
{
  ...
  while (peekChar(parser) != '\0')
  {
    ...
    switch (c)
    {
      ...
      case ';':
      {
          makeToken(parser, TOKEN_SEMICOLON);
          return;
      }
#endif
      case ' ':
      ...
      default:
        ...
        else
        {
          ...
          else
          {
            ...
            // meaningful code units in Luna, the lexer works on raw bytes,
...
// Matches one or more newlines. Returns true if at least one was found.
static bool matchSemicolonSIG(Compiler* compiler)
{
    if (!match(compiler, TOKEN_SEMICOLON)) return false;

    while (match(compiler, TOKEN_SEMICOLON));
    return true;
}

// Discards any newlines starting at the current token.
static void ignoreSemicolonsSIG(Compiler* compiler)
{
    matchSemicolonSIG(compiler);
}

static void allowSemicolonBeforeLineSIG(Compiler* compiler, const char* errorMessage)
{
    //if (peek(compiler) == TOKEN_SEMICOLON)
    {
        //consume(compiler, TOKEN_SEMICOLON, errorMessage);
        ignoreSemicolonsSIG(compiler);
    }
...
{
  xssByteBufferWrite(compiler->parser->env, &compiler->fn->code, (uint8_t)byte);
  
  // Assume the instruction is associated with the most recently consumed token.
  xssIntBufferWrite(compiler->parser->env, &compiler->fn->debug->sourceLines,
...
static int declareVariable(Compiler* compiler, Token* token)
{
  ...
  {
    int line = -1;
    int symbol = xssDefineVariable(compiler->parser->env,
...
static Variable resolveName(Compiler* compiler, const char* name, int length)
{
  ...
  variable.index = xssSymbolTableFind(&compiler->parser->module->variableNames,
                                       name, length);
  return variable;
}
                          ...
                          const char* debugName, int debugNameLength)
{
  ...
  {
    compiler->parser->env->compiler = compiler->parent;
    return NULL;
  }
  ...
  xssFunctionBindName(compiler->parser->env, compiler->fn,
  ...
  #if LUNA_DEBUG_DUMP_COMPILED_CODE
    xssDumpCode(compiler->parser->env, compiler->fn);
  #endif
...
static bool finishBlock(Compiler* compiler)
{
  ...
  {
    definition(compiler);
    //allowSemicolonBeforeLineSIG(compiler, "Expect newline after statement.");
...
{
  return xssSymbolTableEnsure(compiler->parser->env,
      &compiler->parser->env->methodNames, name, length);
}
...
{
  int symbol = xssSymbolTableFind(&compiler->parser->module->variableNames,
                                   name, strlen(name));
...
static void field(Compiler* compiler, bool canAssign)
{
  ...
  else
  {
    ...
    field = xssSymbolTableEnsure(compiler->parser->env, &enclosingClass->fields,
...
static void name(Compiler* compiler, bool canAssign)
{
  ...
  // on this.
  if (xssIsLocalName(token->start) && getEnclosingClass(compiler) != NULL)
  {
    loadThis(compiler);
  ...
  if (variable.index == -1)
  {
    ...
    variable.index = xssDeclareVariable(compiler->parser->env,
                                         compiler->parser->module,
                                         token->start, token->length,
                                         token->line);
...
static void forStatement(Compiler* compiler)
{
  ...
  // actually get names that aren't valid Luna identfiers, but that's the basic
  // idea.
  //
...
//
// Construction is a two-stage process in Luna that involves two separate
                         ...
                         const char* name, int length)
{
  ...
  xssIntBufferWrite(compiler->parser->env, methods, symbol);
  return symbol;
}
...
static bool method(Compiler* compiler, Variable classVariable)
{
  ...
  if (isForeign)
  {
    ...
    emitConstant(compiler, xssNewStringLength(compiler->parser->env,
                                               fullSignature, length));

    // We don't need the function we started compiling in the parameter list
    // any more.
    methodCompiler.parser->env->compiler = methodCompiler.parent;
  }
  else
  {
...
static void classDefinition(Compiler* compiler, bool isForeign)
{
  ...
  Value classNameString = xssNewStringLength(compiler->parser->env,
  ...
  // bytecode will be adjusted by [xssBindMethod] to take inherited fields
  // into account.
  xssSymbolTableInit(&classInfo.fields);
  
  // Set up symbol buffers to track duplicate static and instance methods.
  xssIntBufferInit(&classInfo.methods);
  xssIntBufferInit(&classInfo.staticMethods);
  ...
  xssSymbolTableClear(compiler->parser->env, &classInfo.fields);
  xssIntBufferClear(compiler->parser->env, &classInfo.methods);
  xssIntBufferClear(compiler->parser->env, &classInfo.staticMethods);
  compiler->enclosingClass = NULL;
  popScope(compiler);
}
...
// ObjModule in env->lastModule. Then, for Bar and Baz, we:
//
...
static void import(Compiler* compiler)
{
  ...
  do
  {
    ...
    int sourceVariableConstant = addConstant(compiler,
          xssNewStringLength(compiler->parser->env,
...
void definition(Compiler* compiler)
{
  ...
  {
    consume(compiler, TOKEN_CLASS, "Expect 'class' after 'ffi'.");
...
ObjFn* xssCompile(xssVm env, ObjModule* module, const char* source,
...
void xssBindMethodCode(ObjClass* classObj, ObjFn* fn)
{
  int ip = 0;
  for (;;)
  {
    ...
    switch (instruction)
    {
      ...
      case CODE_CLOSURE:
      {
        ...
        xssBindMethodCode(classObj, AS_FN(fn->constants.data[constant]));
        break;
      }
...
void xssMarkCompiler(xssVm env, Compiler* compiler)
{
  xssGrayValue(env, compiler->parser->current.value);
  xssGrayValue(env, compiler->parser->previous.value);
  xssGrayValue(env, compiler->parser->next.value);

  // Walk up the parent chain to mark the outer compilers too. The VM only
  // tracks the innermost one.
  do
  {
    xssGrayObj(env, (Obj*)compiler->fn);
    xssGrayObj(env, (Obj*)compiler->constants);
    xssGrayObj(env, (Obj*)compiler->attributes);
    
    if (compiler->enclosingClass != NULL)
    {
      xssBlackenSymbolTable(env, &compiler->enclosingClass->fields);

      if(compiler->enclosingClass->methodAttributes != NULL) 
      {
        xssGrayObj(env, (Obj*)compiler->enclosingClass->methodAttributes);
      }
      if(compiler->enclosingClass->classAttributes != NULL) 
      {
        xssGrayObj(env, (Obj*)compiler->enclosingClass->classAttributes);
      }
    }
...
static void disallowAttributes(Compiler* compiler)
{
  if (compiler->numAttributes > 0)
  {
    ...
    xssMapClear(compiler->parser->env, compiler->attributes);
    compiler->numAttributes = 0;
  }
}
...
{
  xssVm env = compiler->parser->env;

  if(IS_OBJ(group)) xssPushRoot(env, AS_OBJ(group));
  if(IS_OBJ(key))   xssPushRoot(env, AS_OBJ(key));
  if(IS_OBJ(value)) xssPushRoot(env, AS_OBJ(value));

  Value groupMapValue = xssMapGet(compiler->attributes, group);
  if(IS_UNDEFINED(groupMapValue)) 
  {
    groupMapValue = OBJ_VAL(xssNewMap(env));
    xssMapSet(env, compiler->attributes, group, groupMapValue);
  }
  ...
  Value keyItemsValue = xssMapGet(groupMap, key);
  if(IS_UNDEFINED(keyItemsValue)) 
  {
    keyItemsValue = OBJ_VAL(xssNewList(env, 0));
    xssMapSet(env, groupMap, key, keyItemsValue);
  }

  //keyItems.add(value)
  ObjList* keyItems = AS_LIST(keyItemsValue);
  xssValueBufferWrite(env, &keyItems->elements, value);

  if(IS_OBJ(group)) xssPopRoot(env);
  if(IS_OBJ(key))   xssPopRoot(env);
...
static void copyAttributes(Compiler* compiler, ObjMap* into)
{
  ...
  for(uint32_t attrIdx = 0; attrIdx < compiler->attributes->capacity; attrIdx++)
  {
    ...
    xssMapSet(env, into, attrEntry->key, attrEntry->value);
  }
  
  xssMapClear(env, compiler->attributes);
}
            ...
            bool isStatic, const char* fullSignature, int32_t length) 
{
  ...
  ObjMap* methodAttr = xssNewMap(env);
  xssPushRoot(env, (Obj*)methodAttr);
  ...
  if(compiler->enclosingClass->methodAttributes == NULL) {
    compiler->enclosingClass->methodAttributes = xssNewMap(env);
  }
  
  // Store the method attributes in the class map
  Value key = xssNewStringLength(env, fullSignatureWithPrefix, fullLength);
  xssMapSet(env, compiler->enclosingClass->methodAttributes, key, OBJ_VAL(methodAttr));
