...
_ARX afxError ArxAcquireTerrain(arxRenderware rwe, arxTerrainConfig const* cfg, arxTerrain* terrain)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_RWE, 1, &rwe);

    afxClass* cls = (afxClass *)_ArxRweGetTerClass(rwe);
    AFX_ASSERT_CLASS(cls, afxFcc_TER);

    if (AfxAcquireObjects(cls, 1, (afxObject*)terrain, (void const*[]) { rwe, cfg }))
    {
        AfxThrowError();
        return err;
    }
...
_ARX afxError ArxGenerateTerrain(arxRenderware rwe, afxWarp const whd, arxTerrain* terrain)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_RWE, 1, &rwe);
    ...
    arxMesh msh;
    //msh = AfxBuildPlaneMesh(sim->rwe, 256, 256, 256, 256);
    //msh = AfxBuildParallelepipedMesh(sim->rwe, AfxWhd(10, 10, 10), 10, 10);
    //msh = AfxBuildCubeMesh(sim->rwe, 10, 1);
    ArxBuildGridMesh(rwe, 100, 100, 1000, 1000, &msh);
    //msh = AfxBuildGridMesh(sim->rwe, 10, 10, 10, 10);
    //msh = AfxBuildDiscMesh(sim->rwe, 10, 10);
    //AfxInvertMeshTopology(msh);

    arxTerrain ter;
    arxTerrainConfig terc = { 0 };
    ArxAcquireTerrain(rwe, &terc, &ter);
...
_ARX afxError ArxGenerateHeightmappedTerrain(arxRenderware rwe, afxUri const* uri, arxTerrain* terrain)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_RWE, 1, &rwe);
    ...
    arxMesh msh;
    ArxBuildGridMesh(rwe, tga.width, tga.height, tga.width * 10, tga.height * 10, &msh);
