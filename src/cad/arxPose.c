...
_ARX afxError ArxAcquirePoses(arxRenderware rwe, afxUnit cnt, arxPoseInfo const artCnt[], arxPose pose[])
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_RWE, 1, &rwe);

    afxClass* cls = (afxClass*)_ArxRweGetPoseClass(rwe);
    AFX_ASSERT_CLASS(cls, afxFcc_POSE);

    if (AfxAcquireObjects(cls, cnt, (afxObject*)pose, (void const*[]) { rwe, artCnt }))
        AfxThrowError();
