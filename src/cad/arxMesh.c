...
ASX afxError AfxAcquireMeshTopology(arxRenderware rwe, arxMeshBlueprint const* blueprint, arxMeshTopology* topology);
...
_ARX arxRenderware ArxGetMeshHost(arxMesh msh)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_MSH, 1, &msh);
...
_ARX afxError _ArxMshCtorCb(arxMesh msh, void** args, afxUnit invokeNo)
{
    ...
    if (!AfxCatalogStrings(_ArxRweGetModelUrnStringBase(rwe), 1, &blueprint->urn.s, &msh->urn))
        AfxThrowError();
    ...
    else if (mtlCnt != (afxUnit)AfxCatalogStrings(_ArxRweGetMorphTagStringBase(rwe), mtlCnt, blueprint->materials, msh->materials))
        AfxThrowError();

    // Temporary workaround to declare materials.
    // TODO: Make it adequate. Transfer to mesh rig?
    arxMaterial materials[256];
    ArxDeclareMaterials(rwe, mtlCnt, msh->materials, materials);
    ...
    else if (biasCnt != (afxUnit)AfxCatalogStrings(_ArxRweGetPivotTagStringBase(rwe), biasCnt, blueprint->biases, msh->biasId))
        AfxThrowError();
    ...
    else if (attrCnt != (afxUnit)AfxCatalogStrings(_ArxRweGetMorphTagStringBase(rwe), attrCnt, blueprint->attrs, msh->attrIds))
        AfxThrowError();
    ...
    else if (morphCnt != (afxUnit)AfxCatalogStrings(_ArxRweGetMorphTagStringBase(rwe), morphCnt, blueprint->morphTags, msh->morphTags))
        AfxThrowError();
...
_ARX afxUnit ArxEnumerateMeshes(arxRenderware rwe, afxUnit first, afxUnit cnt, arxMesh meshes[])
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_RWE, 1, &rwe);
    afxClass const* cls = _ArxRweGetMshClass(rwe);
    AFX_ASSERT_CLASS(cls, afxFcc_MSH);
    return AfxEnumerateObjects(cls, first, cnt, (afxObject*)meshes);
}

_ARX afxError ArxBuildMeshes(arxRenderware rwe, afxUnit cnt, arxMeshBlueprint const blueprints[], arxMesh meshes[])
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_RWE, 1, &rwe);
    ...
    afxClass* cls = (afxClass*)_ArxRweGetMshClass(rwe);
    AFX_ASSERT_CLASS(cls, afxFcc_MSH);

    if (AfxAcquireObjects(cls, cnt, (afxObject*)meshes, (void const*[]) { rwe, blueprints }))
    {
        AfxThrowError();
        return err;
    }
