...
_AMX afxM4d* AfxGetCompositeMatrices(afxPoseBuffer const wp, afxUnit baseArtIdx)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT(wp);
    AFX_ASSERT_RANGE(wp->matCnt, baseArtIdx, 1);
    return &wp->composite[baseArtIdx];
}

_AMX afxM4d* AfxGetWorldMatrices(afxPoseBuffer const wp, afxUnit baseArtIdx)
{
...
_AMX afxUnit AfxGetPoseBufferCapacity(afxPoseBuffer const wp)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT(wp);
    return wp->matCnt;
...
_AMX afxError _AmxPosbCtorCb(afxPoseBuffer posb, void** args, afxUnit invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &posb, afxFcc_POSB);
...
_AMX afxError AfxAcquirePoseBuffers(afxSimulation sim, afxUnit cnt, afxUnit const artCnt[], afxBool const excludeComposite[], afxPoseBuffer wp[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &sim, afxFcc_SIM);
