...
_ASX afxError ArxUploadModel(arxRenderware rwe, afxString const* urn, afxStream in, arxModel* model)
{
    afxError err = NIL;
...
#pragma pop(pack)
    ...
    ArxAssembleModels(rwe, 1, &mdlb, &mdl);
    ...
    ArxBuildMeshes(rwe, mdlHdr.mshCnt, mshbs, meshes);
...
_ASX afxError ArxLoadModel(arxRenderware rwe, afxString const* urn, afxUri const* uri, arxModel* model)
{
    afxError err = NIL;
    afxStream iob;
    afxStreamInfo info = { 0 };
    ...
    ArxUploadModel(rwe, urn, iob, model);
    AfxDisposeObjects(1, &iob);
