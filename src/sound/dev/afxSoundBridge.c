...
_ASX afxUnit AfxGetSoundBridgePort(afxSoundBridge sdge)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &sdge, afxFcc_SDGE);
    return sdge->portIdx;
...
_ASX afxUnit _AsxCountSoundQueues(afxSoundBridge sdge, afxUnit baseQueIdx)
{
    afxError err = AFX_ERR_NONE;
    /// sdge must be a valid afxSoundBridge handle.
    AfxAssertObjects(1, &sdge, afxFcc_SDGE);
...
_ASX afxBool _AsxGetSoundQueue(afxSoundBridge sdge, afxUnit queIdx, afxSoundQueue* queue)
{
    afxError err = AFX_ERR_NONE;
    /// sdge must be a valid afxSoundBridge handle.
    AfxAssertObjects(1, &sdge, afxFcc_SDGE);
...
_ASX afxError _AsxWaitForIdleSoundQueue(afxSoundBridge sdge, afxUnit queIdx)
{
    afxError err = AFX_ERR_NONE;
    /// sdge must be a valid afxSoundBridge handle.
    AfxAssertObjects(1, &sdge, afxFcc_SDGE);
...
_ASX afxUnit _AsxSubmitSoundCommands(afxSoundBridge sdge, asxSubmission const* ctrl, afxUnit cnt, asxTracker trackers[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &sdge, afxFcc_SDGE);
    AFX_ASSERT(cnt);
...
_ASX afxUnit _AsxSubmitSoundOutputFlushes(afxSoundBridge sdge, asxFlush const* ctrl, afxUnit cnt, afxSoundOutput outputs[], afxUnit const bufIdx[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &sdge, afxFcc_SDGE);
    AFX_ASSERT(cnt);
...
_ASX afxUnit _AsxSubmitTransferences(afxSoundBridge sdge, asxTransference const* ctrl, afxUnit opCnt, void const* ops)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &sdge, afxFcc_SDGE);
    AFX_ASSERT(opCnt);
...
_ASX afxError _AsxSdgeStdCtorCb(afxSoundBridge sdge, void** args, afxUnit invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &sdge, afxFcc_SDGE);
    ...
    AFX_ASSERT_RANGE(AfxCountSoundPorts(sdev), cfg->portIdx, 1);
