...
_ASX afxSoundSystem AfxGetAudioSinkContext(afxSink sink)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_ASIO, 1, &sink);
    afxSoundSystem ssys = AfxGetProvider(sink);
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    return ssys;
}

_ASX afxSoundDevice AfxGetAudioSinkDevice(afxSink sink)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_ASIO, 1, &sink);
    afxSoundSystem ssys = AfxGetAudioSinkContext(sink);
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    afxSoundDevice sdev = AfxGetProvider(sink);
    AFX_ASSERT_OBJECTS(afxFcc_SDEV, 1, &sdev);
    return sdev;
}

_ASX afxError AfxGetAudioSinkIdd(afxSink sink, afxUnit code, void* dst)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_ASIO, 1, &sink);
...
_ASX afxError AfxMuteAudioSink(afxSink sink, afxBool mute)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_ASIO, 1, &sink);
...
_ASX afxError AfxRequestSinkBuffer(afxSink sink, afxTime timeout, afxUnit *bufIdx)
...
_ASX afxError AfxDiscardSinkBuffer(afxSink sink, afxUnit bufIdx)
{
    afxError err = AFX_ERR_NONE;
    /// sink must be a valid afxSink handle.
...
_ASX afxError _AsxAsioDtorCb(afxSink sink)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_ASIO, 1, &sink);
    afxSoundDevice sdev = AfxGetAudioSinkDevice(sink);
    AFX_ASSERT_OBJECTS(afxFcc_SDEV, 1, &sdev);
...
_ASX afxError _AsxAsioCtorCb(afxSink sink, void** args, afxUnit invokeNo)
{
    afxError err = AFX_ERR_NONE;
    ...
    afxSinkConfig def;
    AfxConfigureAudioSink(ssys, &def);
    ...
    else
    {
        ...
        {
            if (AfxAcquireAudios(ssys, 1, &audi, &sink->buffers[i]))
            {
                AfxThrowError();
...
_ASX afxClassConfig const _ASX_ASIO_CLASS_CONFIG =
{
    .fcc = afxFcc_ASIO,
    .name = "Sink",
    .desc = "Audio Stream Input/Output",
    .fixedSiz = sizeof(AFX_OBJECT(afxSink)),
    .ctor = (void*)_AsxAsioCtorCb,
    .dtor = (void*)_AsxAsioDtorCb
};

////////////////////////////////////////////////////////////////////////////////

_ASX afxError AfxConfigureAudioSink(afxSoundSystem ssys, afxSinkConfig* cfg)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    AFX_ASSERT(cfg);
    *cfg = (afxSinkConfig) { 0 };
    cfg->fmt = asxFormat_S16i;
    cfg->freq = 48000; // DVD
...
_ASX afxError AfxOpenAudioSink(afxSoundSystem ssys, afxSinkConfig const* cfg, afxSink* sink)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    AFX_ASSERT(cfg);

    afxClass* cls = (afxClass*)AsxGetAudioSinkClass(ssys);
    AFX_ASSERT_CLASS(cls, afxFcc_ASIO);
    afxSink snk;
    afxBool record = FALSE;

    if (AfxAcquireObjects(cls, 1, (afxObject*)&snk, (void const*[]) { ssys, cfg, &record }))
    {
        AfxThrowError();
        return err;
    }
