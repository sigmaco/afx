...
_ASXINL afxSoundSystem AfxGetAudioContext(afxAudio wav)
{
    afxError err;
    AFX_ASSERT_OBJECTS(afxFcc_WAV, 1, &wav);
    afxSoundSystem ssys = AfxGetProvider(wav);
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    return ssys;
}

_ASXINL void AfxDescribeAudio(afxAudio wav, afxAudioInfo* desc)
{
    afxError err;
    AFX_ASSERT_OBJECTS(afxFcc_WAV, 1, &wav);
    AFX_ASSERT(desc);
...
_ASXINL afxReal AfxGetWaveDuration(afxAudio wav)
{
    afxError err;
    AFX_ASSERT_OBJECTS(afxFcc_WAV, 1, &wav);
...
_ASX void _AsxSanitizeWaveInterval(afxAudio wav, afxWaveInterval const* raw, afxWaveInterval* san)
{
...
_ASX void _AsxSanitizeWaveCopy(afxAudio src, afxAudio dst, afxWaveCopy const* raw, afxWaveCopy* san)
{
    afxError err = AFX_ERR_NONE;
...
_ASX afxError AfxUpdateAudio(afxAudio wav, afxUnit opCnt, afxWaveIo const ops[], void const* src, afxUnit portIdx)
{
    afxResult err = NIL;
    AFX_ASSERT_OBJECTS(afxFcc_WAV, 1, &wav);

    afxSoundBridge sexu;
    afxSoundSystem ssys = AfxGetAudioContext(wav);
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    if (!AfxGetSoundBridges(ssys, portIdx, 1, &sexu))
    {
        AfxThrowError();
        return err;
    }
    ...
    if (_AsxSubmitTransferences(sexu, &transfer, opCnt, ops))
    {
        AfxThrowError();
        return err;
    }
...
#if 0
    if (AfxWaitForIdleSoundQueue(ssys, portIdx, transfer.baseQueIdx))
        AfxThrowError();
#endif
    return err;
}

_ASX afxError AfxDumpAudio(afxAudio wav, afxUnit opCnt, afxWaveIo const ops[], void* dst, afxUnit portId)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_WAV, 1, &wav);
    AFX_ASSERT(((wav->bufFlags & afxBufferFlag_ACCESS_MASK) & afxBufferFlag_R));
    AFX_ASSERT(opCnt);
    AFX_ASSERT(ops);
    AFX_ASSERT(dst);
    ...
    afxSoundSystem ssys = AfxGetAudioContext(wav);
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    if (!AfxGetSoundBridges(ssys, portId, 1, &sexu))
    {
        AfxThrowError();
        return err;
    }
...
_ASX afxError AfxUploadAudio(afxAudio wav, afxUnit opCnt, afxWaveIo const ops[], afxStream in, afxUnit portId)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_WAV, 1, &wav);
...
_ASX afxError AfxDownloadAudio(afxAudio wav, afxUnit opCnt, afxWaveIo const ops[], afxStream out, afxUnit portId)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_WAV, 1, &wav);
...
_ASX afxError AfxPrintAudio(afxAudio wav, afxWaveInterval const* op, afxUri const* uri)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_WAV, 1, &wav);
    ...
    if (op) _AsxSanitizeWaveInterval(wav, op, &op2.seg);
    else
    {
    ...
    afxSoundSystem ssys = AfxGetAudioContext(wav);
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    if (AfxWaitForSoundBridge(ssys, 0, 0))
        AfxThrowError();
...
_ASX afxError _AsxWavStdDtorCb(afxAudio wav)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_WAV, 1, &wav);
...
_ASX afxError _AsxWavStdCtorCb(afxAudio wav, void** args, afxUnit invokeNo)
{
    afxResult err = NIL;
    AFX_ASSERT_OBJECTS(afxFcc_WAV, 1, &wav);
    ...
    switch (spec->fmt)
    {
    case asxFormat_A32f: bps = 32; break;
    case asxFormat_S32f: bps = 32; break;
    case asxFormat_A32i: bps = 32; break;
    case asxFormat_S32i: bps = 32; break;
    case asxFormat_A24i: bps = 24; break;
    case asxFormat_S24i: bps = 24; break;
    case asxFormat_A16i: bps = 16; break;
    case asxFormat_S16i: bps = 16; break;
    case asxFormat_A8i: bps = 8; break;
...
_ASX afxClassConfig const _ASX_WAV_CLASS_CONFIG =
{
    .fcc = afxFcc_WAV,
    .name = "Audio",
...
_ASX afxError AfxAcquireAudios(afxSoundSystem ssys, afxUnit cnt, afxAudioInfo const info[], afxAudio buffers[])
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    AFX_ASSERT(buffers);
    ...
    afxClass* cls = (afxClass*)AsxGetAudioClass(ssys);
    AFX_ASSERT_CLASS(cls, afxFcc_WAV);

    afxBool allocOnSysMem = TRUE;

    if (AfxAcquireObjects(cls, cnt, (afxObject*)buffers, (void const*[]) { ssys, (void*)info, &allocOnSysMem }))
        AfxThrowError();

    return err;
}

_ASX afxError AsxLoadAudios(afxSoundSystem ssys, afxUnit cnt, afxUri const uris[], afxAudio buffers[])
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_SSYS, 1, &ssys);
    AFX_ASSERT(buffers);
    ...
    */
    ...
    for (afxUnit fIdx = 0; fIdx < cnt; fIdx++)
    {
        ...
        if (AfxAcquireAudios(ssys, 1, &wavi, &buffers[fIdx]))
            AfxThrowError();
        ...
        //_AsxUploadAudio(buffers[fIdx], &iop, iob);
        AfxWaitForSoundBridge(ssys, 0, 0);
        
        _AsxUpdateAudio(buffers[fIdx], &iop, data);
