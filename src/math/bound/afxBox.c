...
_AFXINL void AfxEmboxTriangles(afxBox* bb, afxUnit triCnt, afxUnit const indices[], afxUnit idxStride, afxV3d const vertices[])
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT(vertices);
    AFX_ASSERT(bb);
...
_AFXINL void AfxEmboxAabbs(afxBox* bb, afxUnit cnt, afxBox const other[])
{
    ...
    for (afxUnit i = 0; i < cnt; i++)
    {
        AfxV3dMin(bb->min, bb->min, other[i].min);
        AfxV3dMax(bb->max, bb->max, other[i].max);
    }
}
...
_AFXINL void AfxEmboxSpheres(afxBox* bb, afxUnit cnt, afxSphere const sph[])
{
    ...
    for (afxUnit i = 0; i < cnt; i++)
    {
        AfxEmboxAabbs(bb, 1, (afxBox[]) { AfxGetSphereAabb(sph[i]) });
    }
...
_AFXINL void AsxGenerateAabbEdges(afxUnit cnt, afxBox const aabbs[], afxV3d vertices[][8], afxUnit16 indices16[][24], afxUnit32 indices32[][24], afxUnit* outVtxCnt, afxUnit* outIdxCnt)
