...
ASX afxError AfxWriteMappedStrings(afxStream out, afxString* sdb, afxUnit cnt, afxString const strings[]);
ASX afxError AfxReadMappedStrings(afxStream in, afxString const* sdb, afxUnit cnt, afxString strings[]);
...
_ASX afxError AfxSerializeCurves(afxStream out, afxUnit cnt, afxCurve curves[])
{
    afxError err = NIL;
...
_ASX afxError AfxSerializeMotions(afxStream out, afxString* sdb, afxUnit cnt, afxMotion motions[])
{
    afxError err = NIL;
    AFX_ASSERT_OBJECTS(afxFcc_IOB, 1, &out);
...
_ASX afxError AfxSerializeAnimation(afxAnimation ani, afxStream out)
{
    afxError err = NIL;
    AFX_ASSERT_OBJECTS(afxFcc_IOB, 1, &out);
...
_ASX afxError AfxArchiveAnimation(afxAnimation ani, afxUri const* uri)
{
    afxError err = NIL;
    afxStream iob;
...
_ASX afxError AfxUploadAnimation(afxSimulation sim, afxString const* urn, afxStream in, afxAnimation* animation)
{
    afxError err = NIL;
...
#pragma pop(pack)
    ...
    afxMotionBlueprint* motbs = AfxRequestFromArena(&arena, sizeof(motbs[0]), aniHdr.motCnt, NIL, 1);
    ...
    for (afxUnit i = 0; i < aniHdr.motCnt; i++)
    {
        ...
        afxCurveInfo* curis = AfxRequestFromArena(&arena, sizeof(curis[0]), motHdr->totalCurCnt, NIL, 0);
        afxCurve curvesStatic[256];
        afxCurve* curves = curvesStatic;// AfxRequestFromArena(&arena, motHdr->totalCurCnt * sizeof(curves[0]));
...
_ASX afxError AfxLoadAnimation(afxSimulation sim, afxString const* urn, afxUri const* uri, afxAnimation* animation)
{
    afxError err = NIL;
    afxStream iob;
    afxStreamInfo info = { 0 };
