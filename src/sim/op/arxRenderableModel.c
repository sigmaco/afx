...
_ASX afxError AfxBufferizeMesh(afxMesh msh, afxUnit morphIdx, avxVertexCache* vtxCache, avxIndexCache* idxCache)
{
    ...
    else
    {
        ...
        avxBuffer buf;
        afxSimulation sim = AfxGetProvider(AfxGetProvider(msh));
...
_ARX afxError ArxCmdRenderMeshes(arxRenderContext rctx, afxUnit cnt, afxMesh meshes[])
{
    afxError err = AFX_ERR_NONE;
    ...
    for (afxUnit mshIdx = 0; mshIdx < cnt; mshIdx++)
    {
        ...
        if (!AfxGetPoolUnit(&rctx->visualMeshes, mshId, &instMsh))
        {
            ...
            AfxBufferizeMesh(msh, 0, &instMsh->vtxCache, &instMsh->idxCache);
        }
...
_ARX afxError ArxCmdRenderModels(arxRenderContext rctx, afxUnit cnt, afxModel models[])
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_RCTX, 1, &rctx);
    ...
    for (afxUnit mdlIdx = 0; mdlIdx < cnt; mdlIdx++)
    {
        ...
        if (!AfxGetPoolUnit(&rctx->visualModels, mdlId, &instMdl))
        {
            ...
            {
                afxMesh msh = NIL;
                AfxGetRiggedMeshes(mdl, rigIdx, 1, &msh);

                afxMeshRigInfo rig;
                AfxDescribeMeshRig(mdl, rigIdx, &rig);
...
afxBool RenderCells(arxRenderContext rctx, afxDrawContext dctx, avxVertexInput vin, afxTerrain ter, afxFrustum* frustum, afxBool showFaces, afxBool showDbgLines)
{
    afxBool result;
...
_ARX afxError ArxCmdRenderTerrain(arxRenderContext rctx, afxTerrain ter)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_RCTX, 1, &rctx);
    ...
    afxMesh msh;
    if (AfxGetTerrainMeshes(ter, 0, 1, &msh))
    {
        afxMeshInfo mshi;
        AfxDescribeMesh(msh, &mshi);
        ...
        AfxBufferizeMesh(msh, 0, &vtxCache, &idxCache); // request drawable cache.
