...
_AMX afxError AfxWriteMappedStrings(afxStream out, afxString* sdb, afxUnit cnt, afxString const strings[])
{
    afxError err = NIL;
    AFX_ASSERT_OBJECTS(afxFcc_IOB, 1, &out);
    AFX_ASSERT(strings);
...
_AMX afxError AfxReadMappedStrings(afxStream in, afxString const* sdb, afxUnit cnt, afxString strings[])
{
    afxError err = NIL;
    AFX_ASSERT_OBJECTS(afxFcc_IOB, 1, &in);
    AFX_ASSERT(strings);
...
_AMX afxError AfxArchiveMeshes(afxStream out, afxString* sdb, afxUnit cnt, afxMesh meshes[])
{
    afxError err = NIL;
    AFX_ASSERT_OBJECTS(afxFcc_IOB, 1, &out);
...
_AMX afxError AfxDownloadModel(afxModel mdl, afxStream out)
{
    afxError err = NIL;
    AFX_ASSERT_OBJECTS(afxFcc_IOB, 1, &out);
    ...
    for (afxUnit i = 0; i < mdlHdr.rigCnt; i++)
    {
        ...
        if (msh)
        {
            ...
            {
                if (msh == mdl->rigs[j].msh)
                {
                    skip = TRUE; // must skip this mesh because it was already included before.
                    break;
                }
            }

            if (skip) continue;  // skip mesh if it was already included before.
            else
            {
...
_AMX afxError AfxArchiveModel(afxModel mdl, afxUri const* uri)
{
    afxError err = NIL;
    afxStream iob;
...
_AMX afxError AfxUploadModel(afxSimulation sim, afxString const* urn, afxStream in, afxModel* model)
{
    afxError err = NIL;
...
#pragma pop(pack)
    ...
    AfxAssembleMeshes(sim->din, mdlHdr.mshCnt, mshbs, meshes);
    ...
    for (afxUnit i = 0; i < mdlHdr.rigCnt; i++)
    {
        ...
        AfxRigMeshes(mdl, i, 1, &meshes[mshrHdr.mshIdx], mdl);
        mdl->rigs[i].flags = mshrHdr.flags;
    }
...
_AMX afxError AfxLoadModel(afxSimulation sim, afxString const* urn, afxUri const* uri, afxModel* model)
{
    afxError err = NIL;
    afxStream iob;
    afxStreamInfo info = { 0 };
