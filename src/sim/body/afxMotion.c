...
_AMX afxMask AfxGetMotionFlags(afxMotion mot)
{
...
_AMXINL afxBool AfxFindMotionTransform(afxMotion mot, afxString const* seqId, afxUnit *seqIdx)
{
    ...
    //afxStringBase strb = AvxGetPivotTagStringBase(AfxGetProvider(mot));
...
_AMXINL afxBool AfxFindMotionVector(afxMotion mot, afxString const* seqId, afxUnit *seqIdx)
{
    ...
    afxStringBase strb = AvxGetMorphTagStringBase(AfxGetProvider(mot));
...
_AMX void AfxUpdateMotionVectors(afxMotion mot, afxUnit baseSeqIdx, afxUnit seqCnt, afxVectorialMotion const vectors[], afxUnit fetchRate)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_MOT, 1, &mot);
...
_AMX void AfxUpdateMotionTransforms(afxMotion mot, afxUnit baseSeqIdx, afxUnit seqCnt, afxPivotalMotion const transforms[], afxUnit fetchRate)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT_OBJECTS(afxFcc_MOT, 1, &mot);
...
_AMX afxError _AmxMotCtorCb(afxMotion mot, void** args, afxUnit invokeNo)
{
    ...
    else if (!AfxCatalogStrings(AvxGetMorphTagStringBase(AfxGetSimulationDrawInput(sim)), vecCnt, motb->vectors, mot->vectors)) // AfxRegisterMorphTags is temp
        AfxThrowError();
    ...
    else if (!AfxCatalogStrings(AvxGetPivotTagStringBase(AfxGetSimulationDrawInput(sim)), pivotCnt, motb->pivots, mot->pivots))
        AfxThrowError();
...
_AMX afxClassConfig const _AMX_MOT_CLASS_CONFIG =
{
    .fcc = afxFcc_MOT,
    .name = "Motion",
...
_AMX afxError AfxAssembleMotions(afxSimulation sim, afxUnit cnt, afxMotionBlueprint const blueprints[], afxMotion motions[])
{
    ...
    afxClass* cls = (afxClass*)_AmxGetMotionClass(sim);
