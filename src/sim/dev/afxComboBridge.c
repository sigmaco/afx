...
_AMX afxUnit AfxGetMathBridgePort(afxComboBridge mdge)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &mdge, afxFcc_MDGE);
    return mdge->portIdx;
...
_AMX afxUnit _AmxCountMathQueues(afxComboBridge mdge, afxUnit baseQueIdx)
{
    afxError err = AFX_ERR_NONE;
    /// mdge must be a valid afxComboBridge handle.
    AfxAssertObjects(1, &mdge, afxFcc_MDGE);
    AFX_ASSERT_RANGE(mdge->queCnt, baseQueIdx, 1);
    return mdge->queCnt - baseQueIdx;
}

_AMX afxBool _AmxGetMathQueue(afxComboBridge mdge, afxUnit queIdx, afxMathQueue* queue)
{
    afxError err = AFX_ERR_NONE;
    /// mdge must be a valid afxComboBridge handle.
    AfxAssertObjects(1, &mdge, afxFcc_MDGE);
    /// queIdx must be less than the value of queCnt for the bridge indicated by portIdx when device context was created.
    AFX_ASSERT_RANGE(mdge->queCnt, queIdx, 1);
    afxBool rslt;
...
_AMX afxError _AmxWaitForIdleMathQueue(afxComboBridge mdge, afxUnit queIdx)
{
    afxError err = AFX_ERR_NONE;
    /// mdge must be a valid afxComboBridge handle.
    AfxAssertObjects(1, &mdge, afxFcc_MDGE);
...
_AMX void _AmxBeginMathQueueDebugScope(afxComboBridge mdge, afxUnit queIdx, afxString const* name, afxColor const color)
{
    afxError err = AFX_ERR_NONE;
    /// mdge must be a valid afxComboBridge handle.
    AfxAssertObjects(1, &mdge, afxFcc_MDGE);
    afxUnit queCnt = mdge->queCnt;
    AFX_ASSERT_RANGE(queCnt, queIdx, 1);
    

}

_AMX void _AmxEndMathQueueDebugScope(afxComboBridge mdge, afxUnit queIdx)
{
    afxError err = AFX_ERR_NONE;
    /// mdge must be a valid afxComboBridge handle.
    AfxAssertObjects(1, &mdge, afxFcc_MDGE);
...
_AMX void _AmxPushMathQueueDebugLabel(afxComboBridge mdge, afxUnit queIdx, afxString const* name, afxColor const color)
{
    afxError err = AFX_ERR_NONE;
    /// mdge must be a valid afxComboBridge handle.
    AfxAssertObjects(1, &mdge, afxFcc_MDGE);
...
_AMX afxUnit _AmxSubmitMathCommands(afxComboBridge mdge, amxSubmission const* ctrl, afxUnit cnt, avxCmdb cmdbs[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &mdge, afxFcc_MDGE);
    AFX_ASSERT(cnt);
...
_AMX afxUnit _AmxSubmitMathWorkRequest(afxComboBridge mdge, afxUnit cnt, amxQueueOp const subm[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &mdge, afxFcc_MDGE);
    AFX_ASSERT(cnt);
...
_AMX afxError _AmxMdgeStdDtorCb(afxComboBridge mdge)
{
    ...
    AfxDeallocateInstanceData(mdge, ARRAY_SIZE(stashes), stashes);

    return err;
}

_AMX afxError _AmxMqueStdCtorCb(afxMathQueue mque, void** args, afxUnit invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &mque, afxFcc_MQUE);
...
_AMX afxError _AmxMdgeStdCtorCb(afxComboBridge mdge, void** args, afxUnit invokeNo)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &mdge, afxFcc_MDGE);
    ...
    AFX_ASSERT_RANGE(AfxCountPragmaPorts(mdev), cfg->portIdx, 1);
    ...
    if (AfxAllocateInstanceData(mdge, ARRAY_SIZE(stashes), stashes)) AfxThrowError();
    else
    {
