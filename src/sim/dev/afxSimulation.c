...
_AMX afxUnit AfxCountMathBridges(afxSimulation sim, afxUnit baseBridIdx)
{
    afxError err = AFX_ERR_NONE;
    /// sim must be a valid afxSimulation handle.
    AfxAssertObjects(1, &sim, afxFcc_SIM);
    AFX_ASSERT_RANGE(sim->exuCnt, baseBridIdx, 1);
    return sim->exuCnt - baseBridIdx;
}

_AMX afxBool AfxGetMathBridge(afxSimulation sim, afxUnit portIdx, afxComboBridge* bridge)
{
    afxError err = AFX_ERR_NONE;
    /// sim must be a valid afxSimulation handle.
    AfxAssertObjects(1, &sim, afxFcc_SIM);
    /// portIdx must be one of the bridge indices specified when device was created.
    AFX_ASSERT_RANGE(sim->exuCnt, portIdx, 1);
    afxBool rslt;
...
_AMX afxBool AfxGetMathQueue(afxSimulation sim, afxUnit portIdx, afxUnit queIdx, afxMathQueue* queue)
{
    afxError err = AFX_ERR_NONE;
    /// sim must be a valid afxSimulation handle.
    AfxAssertObjects(1, &sim, afxFcc_SIM);
...
_AMX afxUnit AfxCountMathQueues(afxSimulation sim, afxUnit portIdx, afxUnit baseQueIdx)
{
    afxError err = AFX_ERR_NONE;
    /// sim must be a valid afxSimulation handle.
    AfxAssertObjects(1, &sim, afxFcc_SIM);
    ...
    else
    {
        ...
        AFX_ASSERT_RANGE(queCnt, baseQueIdx, 1);
        queCnt -= baseQueIdx;
    }
    return queCnt;
}

_AMX afxError AfxWaitForSimulation(afxSimulation sim, afxUnit portIdx, afxUnit firstQueIdx, afxUnit queCnt)
{
    afxError err = AFX_ERR_NONE;
    /// sim must be a valid afxSimulation handle.
    AfxAssertObjects(1, &sim, afxFcc_SIM);
    ...
    if (!sim->waitCb)
    {
        ...
        if (portIdx != AFX_INVALID_INDEX)
        {
            ...
            afxUnit queCnt2 = _AmxCountMathQueues(mdge, firstQueIdx) - queCnt;

            if (!queCnt || queCnt == queCnt2)
                _AmxWaitForIdleMathBridge(mdge);
            else for (afxUnit j = 0; j < queCnt2; j++)
                _AmxWaitForIdleMathQueue(mdge, j);
        }
        else for (afxUnit i = 0; i < mdgeCnt; i++)
        {
...
_AMX afxError _AmxSimCtorCb(afxSimulation sim, void** args, afxUnit invokeNo)
{
    ...
    if (config)
    {
        ...
        AFX_ASSERT(config->unitsPerMeter);
        AFX_ASSERT(!AfxV3dIsZero(config->right));
        AFX_ASSERT(!AfxV3dIsZero(config->up));
        AFX_ASSERT(!AfxV3dIsZero(config->back));
...
_AMX afxUnit AfxInvokeSimulations(afxComboDevice mdev, afxUnit first, afxUnit cnt, afxBool(*f)(afxSimulation, void*), void *udd)
{
    afxError err = AFX_ERR_NONE;
...
_AMX afxUnit AfxEvokeSimulations(afxComboDevice mdev, afxBool(*flt)(afxSimulation, void*), void* fdd, afxUnit first, afxUnit cnt, afxSimulation simulations[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &mdev, afxFcc_MDEV);
...
_AMX afxUnit AfxEnumerateSimulations(afxComboDevice mdev, afxUnit first, afxUnit cnt, afxSimulation simulations[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &mdev, afxFcc_MDEV);
    AFX_ASSERT(cnt);
...
_AMX afxError AfxAcquireSimulations(afxUnit mdevId, akxSimulationConfig const* cfg, afxSimulation* simulation)
{
    afxError err = AFX_ERR_NONE;
    afxComboDevice mdev;
...
_AMX afxError AfxConfigureSimulation(afxUnit mdevId, akxSimulationConfig* cfg)
{
    afxError err = AFX_ERR_NONE;
    AFX_ASSERT(mdevId != AFX_INVALID_INDEX);
    afxComboDevice mdev;
