...
ARX void            ArxResetCapstanLocalTime(afxReal localTime, afxUnit cnt, arxCapstan capstans[]);

// Devido a limitação imposto pelo tipo afxReal32, os relógios de uma arxCapstan não podem operar por mais que um dia e meio sem problemas relacionados a perda de precisão.
...
ARX void            ArxRebaseCapstanTimes(afxReal time, afxUnit cnt, arxCapstan capstans[]);
...
ARX void            ArxSetCapstanTimeOnly(afxReal time, afxUnit cnt, arxCapstan capstans[]);
...
ARX afxError        ArxAcquireCapstans(arxScenario scio, arxCapstanConfig const* cfg, afxUnit cnt, arxCapstan capstans[]);
