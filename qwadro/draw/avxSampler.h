...
/*
    Texel filtering is the process of sampling from a texture map in graphics applications. 
    It determines how a final texel (texture pixel) value is computed based on texture coordinates (usually non-integer values).

    Nearest-Neighbor Filtering (Point Sampling) is the simplest form. It just round the texture coordinate to the nearest integer.

    Bilinear Filtering interpolates between the four nearest texels.

    Trilinear Filtering is a bilinear filtering across two mipmap levels, then interpolate between those.

    Anisotropic Filtering (even simplified) is complex, used to reduce blur when textures are viewed at steep angles. 
    A simplified version might take more samples in the direction of the gradient.
    At a high level:
        Compute the texture coordinate derivatives (du/dx, dv/dy).
        Sample multiple texels along the dominant axis.
        Average or weighted-average those samples.

    Due to its complexity and performance cost, anisotropic filtering is usually done in GPU hardware using techniques like:
        Elliptical weighted average (EWA).
        Mip level sampling along the anisotropy direction.
*/
...
#define AVX_DEFAULT_CLEAR_VALUE (avxClearValue){ \
    .rgba[0] = 0.f, \
    .rgba[1] = 0.f, \
    .rgba[2] = 0.f, \
    .rgba[3] = 0.f };
...
#define AVX_DEFAULT_SAMPLER_CONFIG (avxSamplerConfig){ \
    .flags = NIL, \
...
AVX afxError        AvxConfigureSampler
(
    afxDrawSystem dsys, 
...
(
    afxDrawSystem dsys, 
    afxUnit cnt, 
    avxSamplerConfig const cfg[], 
    avxSampler samplers[]
);

AVX afxBool         AvxFindSamplers
(
    afxDrawSystem dsys,
    afxUnit cnt,
    avxSamplerConfig const cfg[],
...
AVX afxDrawSystem   AvxGetSamplerHost
(
    avxSampler      samp
);

AVX afxFlags        AvxGetSamplerFlags
(
    avxSampler samp, 
    afxFlags mask
);

AVX void            AvxDescribeSampler
(
    avxSampler samp, 
    avxSamplerConfig* desc
);

AVX void            AvxGetSamplerBorderColor
(
    avxSampler samp, 
    avxClearValue* val
);

AVX afxBool         AvxIsSamplerYuvCapable
(
    avxSampler samp
);
