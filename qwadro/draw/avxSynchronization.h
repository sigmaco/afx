...
(
    ...
    afxUnit             slot, // the index within the query pool that will contain the results.
...
(
    ...
    afxUnit             slotCnt, // the number of queries. @baseQuery and @queryCnt together define a range of queries.
...
(
    afxDrawContext      dctx,
    avxQueryPool        pool,
    afxUnit             slot, // the query that will contain the timestamp.
    avxBusStage         stage // the stage of the pipeline.
);
