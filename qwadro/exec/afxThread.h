...
/**
    ...
    A afxThread object manages one thread of control within the execution process.
...
AFX void**      AfxGetThreadUdd
(
    afxThread thr
);

/*
    The AfxRunThread() function begins execution of the thread by calling proc(). 
    ...
    (such as on Linux, see the sched_setscheduler documentation for more details).
    Return from the function proc() is equivalent to calling exit() with the argument equal to the return value of proc().
*/

AFX afxError    AfxRunThread
(
    afxThread thr, 
    afxInt(*proc)(void* arg), 
    void* arg
);

/*
    The AfxRequestThreadInterruption() method request the interruption of the thread. 
...
AFX void        AfxRequestThreadInterruption
(
    afxThread thr
);
...
/*
    The AfxIsThreadRunning() method returns true if the thread is running; otherwise returns false.
*/
AFX afxBool     AfxIsThreadRunning
(
    afxThread thr
);

/*
    The AfxIsThreadFinished() method returns true if the thread is finished; otherwise returns false.
*/

AFX afxBool     AfxIsThreadFinished
(
    afxThread thr
);

AFX afxBool     AfxGetThreadExitCode
(
    afxThread thr, 
    afxInt* exitCode
);
...
/*
    The AfxWaitForThreadExit() method blocks the thread until the thread associated with this afxThread object has 
...
AFX afxResult   AfxWaitForThreadExit
(
    afxThread thr, 
    afxResult* exitCode
);
...
// The Qwadro AFX tries to avoid letting users to control other users' threads.
// The following functions affects the currrent caller thread.
...
/*
    The AfxIsPrimeThread() function tests if the currently executing thread is the prime thread.
    The prime thread is the one which bootstrapped the Qwadro. By design, it is expected to be the process' main thread.
*/

AFX afxBool     AfxIsPrimeThread(void);

/*
    The AfxIsUxThread() function tests if the currently executing thread is an UX thread.
*/

AFX afxBool     AfxIsUxThread(void);

/*
    The AfxGetTid() function returns the thread unique identifier of the currently executing thread.
*/

AFX afxUnit32    AfxGetTid(void);

/*
    The AfxShouldThreadBeInterrupted() function returns true if the task running on this thread should be stopped. 
...
/*
    The AfxExitThread() function tells the thread's event loop to exit with a return code.
    After calling this function, the thread leaves the event loop and returns from the call to event loop exec(). 
    The event loop exec() function returns returnCode. 
    By convention, a returnCode of 0 means success, any non-zero value indicates an error.
    Note that unlike the C library function of the same name, this function does return to the caller; it is event processing that stops.
    No event loops will be started anymore in this thread until thread exec() has been called again.
    If the eventloop in thread exec() is not running then the next call to thread exec() will also return immediately.
*/

AFX void        AfxExitThread(afxInt code);

/*
    The AfxQuitThread() function tells the thread's event loop to exit with return code 0 (success). 
...
/*
    The AfxYield() function yields execution of the current thread to another runnable thread, if any. 
...
/*
    The AfxGetThread() function returns a afxThread handle which manages the currently executing thread.
*/
AFX afxBool     AfxGetThread
(
    afxThread* thread
);

/*
    The AfxFindThread() function finds a afxThread handle associated with the currently executing thread.
*/

AFX afxBool     AfxFindThread
(
    afxUnit32 tid, 
    afxThread* thread
);

AFX afxBool     AfxGetPrimeThread
(
    afxThread* thread
);

AFX afxError    AfxAcquireThreads
(
    afxHere const hint, 
    afxThreadConfig const* cfg, 
    afxUnit cnt, 
    afxThread threads[]
);

AFX afxUnit     AfxEnumerateThreads
(
    afxUnit first, 
    afxUnit cnt, 
    afxThread threads[]
...
AFX afxUnit     AfxInvokeThreads
(
    afxUnit first, 
    afxUnit cnt, 
    afxBool(*f)(afxThread, void*), 
    void *udd
);
