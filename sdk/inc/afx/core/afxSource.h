...
AFX_DEFINE_STRUCT(afxSource)
{
    ...
    void* memBuf;                ///< Pointer to the in-memory file.  If not NULL, the source will be read from memory. Set szFile and idFile to NULL. The only file format accepted is a fully formed WEM file, as converted by Wwise.
    afxNat32 memSiz;          ///< Size of the data pointed by pInMemory
