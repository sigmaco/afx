...
_AVX afxError AfxOpenDrawInput(afxNat ddevId, afxDrawInputConfig const* cfg, afxUri const* endpoint, afxDrawInput* input)
{
    afxError err = AFX_ERR_NONE;
    afxDrawDevice ddev;
    ...
    else
    {
        ...
        if (AfxAcquireObjects(cls, 1, (afxObject*)&din, (void const*[]) { ddev, cfg, endpoint })) AfxThrowError();
        else
        {
            AfxAssertObjects(1, &din, afxFcc_DIN);
            AfxAssert(input);
...
_AVX afxError AfxAcquireDrawInput(afxNat ddevId, afxDrawInputConfig const* cfg, afxDrawInput* input)
{
    ...
    if (AfxOpenDrawInput(ddevId, cfg, NIL, &din)) AfxThrowError();
    else
    {
        AfxAssertObjects(1, &din, afxFcc_DIN);
        AfxAssert(input);
