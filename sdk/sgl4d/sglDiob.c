...
_SGL afxError _SglDiobDtor(afxDrawStream diob)
{
    ...
    while (1)
    {
        if (diob->base.state != afxDrawStreamState_PENDING) break;
        else
        {
            // AfxPopLinkage(&diob->base.queue); // we can't do it here. We need wait for draw context to liberate it.
    ...
    AfxDeallocateArena(&diob->base.cmdArena);
...
_SGL afxError _SglDiobCtor(afxDrawStream diob, afxCookie const* cookie)
{
    ...
    diob->base.state = afxDrawStreamState_INITIAL;
    
    diob->base.stdCmds = NIL;
