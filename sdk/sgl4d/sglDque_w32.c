...
_SGL afxError _SglDpuStamp(sglDpuIdd* dpu, _sglQueueingStamp* subm)
{
    ...
    for (afxNat i = 0; i < subm->itemCnt; i++)
    {
        ...
        afxReal* verts = AfxMapBufferRange(dctx->fntDataBuf, 0, 2048, afxBufferAccess_W);
        ...
        AfxUnmapBufferRange(dctx->fntDataBuf);
        ...
        for (afxNat queIter = 0; queIter < queCnt; queIter++)
        {
            ...
            if (AfxTryEnterSlockExclusive(&dque->base.pendingChainSlock))
            {
                ...
                if (ctxEntered)
                {
                    ...
                    _sglQueueing* subm;
                    AfxChainForEveryLinkageB2F(&dque->base.pendingChain, _sglQueueing, chain, subm)
                    {
...
_SGL afxError _SglDctxExecute(afxDrawContext dctx, afxNat cnt, afxExecutionRequest const req[], afxFence fenc)
{
    ...
    {
        _sglQueueingExecute* subm = _AfxDqueRequestArenaSpace(dctx, dque, sizeof(*subm) + (cnt * sizeof(subm->items[0])));
        *subm = (_sglQueueingExecute) { 0 };
        ...
        AfxPushLinkage(&subm->hdr.chain, &dque->base.pendingChain);
        AfxExitSlockExclusive(&dque->base.pendingChainSlock);

        ++j;

        if (j >= dctx->base.openPorts[portIdx].dqueCnt)
            break;
    }
    return err;
}
...
_SGL afxError _SglDctxPresent(afxDrawContext dctx, afxNat cnt, afxPresentationRequest const req[])
{
    ...
    while ((dque = dctx->base.openPorts[portIdx].queues[j]))
    {
        _sglQueueingPresent* subm = _AfxDqueRequestArenaSpace(dctx, dque, sizeof(*subm) + (cnt * sizeof(subm->items[0])));
...
_SGL afxError _SglDctxStamp(afxDrawContext dctx, afxNat cnt, afxPresentationRequest const req[], afxV2d const origin, afxString const* caption)
{
    ...
    while ((dque = dctx->base.openPorts[portIdx].queues[j]))
    {
        _sglQueueingStamp* subm = _AfxDqueRequestArenaSpace(dctx, dque, sizeof(*subm) + (cnt * sizeof(subm->items[0])));
