...
_SGL void _SglDpuBeginSynthesis(sglDpuIdd* dpu, _sglCmdBeginSynthesis const *cmd)
{
    ...
    _SglDpuBindAndSyncCanv(gl, (dpu->activeRasterState.pass.canv != canv), TRUE, GL_DRAW_FRAMEBUFFER, canv);
...
_SGL void _SglDpuRegenerateMipmaps(sglDpuIdd* dpu, _sglCmdRegenerateMipmaps const* cmd)
{
    ...
    _SglBindAndSyncRas(gl, TRUE, TRUE, SGL_LAST_COMBINED_TEXTURE_IMAGE_UNIT, ras);
    gl->GenerateMipmap(ras->glTarget); _SglThrowErrorOccuried();
    _SglBindAndSyncRas(gl, TRUE, FALSE, SGL_LAST_COMBINED_TEXTURE_IMAGE_UNIT, NIL);
...
_SGL void _SglDpuPackRaster(sglDpuIdd* dpu, _sglCmdPackRasterRegions const* cmd)
{
    ...
    afxNat rasMaxRowSiz = AfxMeasureRasterRow(ras, 0);
...
_SGL void _SglDpuCopyRaster(sglDpuIdd* dpu, _sglCmdCopyRasterRegions const* cmd)
{
    ...
    for (afxNat i = 0; i < opCnt; i++)
    {
        ...
        afxNat dstRgnSiz;
        afxNat dstRowSiz;
        void *dstData = AfxOpenRasterRegion(dst, &dstRgn, afxRasterAccess_W, &dstRgnSiz, &dstRowSiz);
        ...
        else
        {
            afxNat srcRowSiz;
            afxNat srcRgnSiz;
            void *srcData = AfxOpenRasterRegion(src, &srcRgn, afxRasterAccess_R, &srcRgnSiz, &srcRowSiz);
...
_SGL void _SglDpuFlipRaster(sglDpuIdd* dpu, _sglCmdFlipRasterRegions const* cmd)
{
    ...
    for (afxNat j = 0; j < rgnCnt; j++)
    {
        ...
        afxNat bpl = AfxMeasureRasterLayer(ras, 0);
    ...
    for (afxNat i = 0; i < dpu->activePip->base.wiringCnt; i++)
    {
        ...
        for (afxNat j = 0; j < lego->base.entryCnt; j++)
        {
            ...
            case AFX_SHD_RES_TYPE_CONSTANT_BUFFER:
            {
                ...
                _SglBindAndSyncBuf(gl, FALSE, GL_UNIFORM_BUFFER, binding, buf, offset, range, 0, /*buf->glUsage ? buf->glUsage : */GL_DYNAMIC_DRAW);
                break;
            }
            ...
            case AFX_SHD_RES_TYPE_COMBINED_IMAGE_SAMPLER:
            {
                ...
                {
                    _SglBindAndSyncRas(gl, reqUpd, TRUE, binding, ras);
                }

                if (entry->type == AFX_SHD_RES_TYPE_SAMPLER || entry->type == AFX_SHD_RES_TYPE_COMBINED_IMAGE_SAMPLER)
                {
                    _SglDpuBindAndSyncSamp(gl, FALSE, binding, samp);
                }
                break;
            }
            default:
...
_SGL void _SglFlushVertexInputStateChanges(sglDpuIdd* dpu, afxBool chkIbo)
{
    afxError err = AFX_ERR_NONE;
    glVmt const* gl = &dpu->gl;
            ...
            if (dpu->activeVin->base.streamCnt > streamIdx)
            {
...
#endif       
                afxNat32 stride = dpu->nextVinBindings.sources[streamIdx].stride;
                ...
                if (updMask & AfxGetBitOffset(streamIdx))
                {
...

                    {
                        ...
                        if (dpu->activeVin->bindings.sources[streamIdx].stride != stride)
                        {
                            dpu->activeVin->bindings.sources[streamIdx].stride = stride;
                            updReq = TRUE;
                        }
                    }
                }

                _SglBindAndSyncBuf(gl, FALSE, GL_ARRAY_BUFFER, streamIdx, buf, offset, range, stride, /*buf->glUsage ? buf->glUsage : */GL_STATIC_DRAW);
            }
        }
    ...
    if (chkIbo)
    {
        ...
        _SglBindAndSyncBuf(gl, FALSE, GL_ELEMENT_ARRAY_BUFFER, 0, buf, off, range, idxSrcSiz, /*buf->glUsage ? buf->glUsage : */GL_STATIC_DRAW);
    }
}
...
_SGL afxError _SglDpuPresentSurf(sglDpuIdd* dpu, afxDrawOutput dout, afxNat outBufIdx)
{
...

    {
        ...
        if (rc != dpu->glrc || dc != dout->dc)
        {
            ...
            dpu->SwapIntervalEXT(0); // causes lag when 1
        }
