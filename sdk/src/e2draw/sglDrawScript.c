...
_SGL void _SglDcmdNextPass(afxDrawScript dscr, afxBool useAuxScripts)
{
    ...
    _afxDscrCmdNextPass *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
...
_SGL void _SglDcmdSetRasterizerState(afxDrawScript dscr, afxPipelineRasterizerState const *state)
{
    ...
    _afxDscrCmdSetRasterizerState *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->state = *state;
...
_SGL void _SglDcmdSetDepthState(afxDrawScript dscr, afxPipelineDepthState const *state)
{
    ...
    _afxDscrCmdSetDepthState *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->state = *state;
...
_SGL void _SglDcmdSetInputAssemblyState(afxDrawScript dscr, afxPipelineInputAssemblyState const *state)
{
    ...
    _afxDscrCmdSetInputAssemblyState *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->state = *state;
...
_SGL void _SglDcmdSetViewports(afxDrawScript dscr, afxNat32 first, afxNat32 cnt, afxViewport const vp[])
{
    ...
    _afxDscrCmdSetVp *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->first = first;
    cmd->cnt = cnt;
...
_SGL void _SglDcmdSetScissors(afxDrawScript dscr, afxNat32 first, afxNat32 cnt, afxRect const rect[])
{
    ...
    _afxDscrCmdSetScissor *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->first = first;
    cmd->cnt = cnt;
...
_SGL void _SglDcmdBindPipeline(afxDrawScript dscr, afxPipeline pip)
{
    ...
    _afxDscrCmdBindPip *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->pip = pip;
...
_SGL void _SglDcmdBindIndexBuffer(afxDrawScript dscr, afxBuffer buf, afxNat32 offset, afxNat32 idxSiz)
{
    ...
    _afxDscrCmdBindIbuf *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->buf = buf;
    cmd->offset = offset;
    cmd->idxSiz = idxSiz;
...
_SGL void _SglDcmdBindBuffers(afxDrawScript dscr, afxNat set, afxNat first, afxNat cnt, afxBuffer buf[], afxNat offset[], afxNat range[])
{
    ...
    _afxDscrCmdBindBuffers *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->set = set;
    cmd->first = first;
    cmd->cnt = cnt;
...
_SGL void _SglDcmdBindTextures(afxDrawScript dscr, afxNat set, afxNat first, afxNat cnt, afxSampler smp[], afxTexture tex[])
{
    ...
    _afxDscrCmdBindTextures *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->set = set;
    cmd->first = first;
    cmd->cnt = cnt;
...
_SGL void _SglDcmdBindVertexBuffers(afxDrawScript dscr, afxNat first, afxNat cnt, afxBuffer buf[], afxSize const offset[])
{
    ...
    _afxDscrCmdBindVbuf *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->first = first;
    cmd->cnt = cnt;
...
_SGL void _SglDcmdDrawIndexed(afxDrawScript dscr, afxNat32 idxCnt, afxNat32 instCnt, afxNat32 firstIdx, afxNat32 vtxOff, afxNat32 firstInst)
{
    ...
    _afxDscrCmdDrawIndexed *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->idxCnt = idxCnt;
    cmd->instCnt = instCnt;
...
_SGL void _SglDcmdDraw(afxDrawScript dscr, afxNat32 vtxCnt, afxNat32 instCnt, afxNat32 firstVtx, afxNat32 firstInst)
{
    ...
    _afxDscrCmdDraw *cmd = AfxRequestArenaSpace(&dscr->cmdArena, sizeof(*cmd));
    AfxAssert(cmd);
    cmd->vtxCnt = vtxCnt;
    cmd->instCnt = instCnt;
    ...
    while (1)
    {
        ...
        else
        {
            ...
            //if (cmd != &dscr->cmdEnd)
            {
                ...
                AfxRecycleArenaSpace(&idd->cmdArena, cmd, cmd->siz);
            }
        }
    }
#else
