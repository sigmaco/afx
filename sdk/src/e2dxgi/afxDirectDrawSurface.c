...
void align_memory(afxMmu mmu, DdsData *dds, DdsImage *surface)
{
    ...
    newSurface.data.pixels = AfxAllocate(mmu, imagesize, 1, AfxHint());
...
{
    unsigned char *tmp = AfxAllocate(mmu, sizeof(afxByte), size, 0, AfxHint());
...
_AFXEXPORT afxBool _AfxDdsLoad(afxMmu mmu, afxUri *filename, afxBool flipImage, DdsData *dds)
{
    ...
    for (int n = 0; n < (dds->cubemap ? 6 : 1); n++)
    {
        ...
        img.data.size = size;
        img.data.pixels = AfxAllocate(mmu, sizeof(afxByte), size, AfxHint());
        ...
        for (int i = 0; i < numMipmaps && (w || h); i++)
        {
            ...
            mipmap.depth = depth;
            mipmap.size = size;
            mipmap.pixels = AfxAllocate(mmu, sizeof(afxByte), size, AfxHint());
