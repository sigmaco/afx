...
_AFX afxError AfxVertexBufferForEachVertex(afxVertexBuffer vbuf, afxNat attrIdx, afxNat baseVtx, afxNat vtxCnt, void(*f)(void const *vtx, void*data), void *data)
{
    ...
    afxNat strIdx = vbuf->attrs[attrIdx].srcIdx;
    afxNat strBase = vbuf->storages[strIdx].base;
    afxNat strRange = vbuf->storages[strIdx].range;
    afxNat strStride = vbuf->storages[strIdx].stride;

    afxNat attrOffset = vbuf->attrs[attrIdx].offset;
    afxNat attrSiz = AfxVertexFormatGetSize(vbuf->attrs[attrIdx].fmt);

    AfxAssertRange(strRange, baseVtx * strStride, vtxCnt * strStride);
    afxByte* src = AfxMapBufferRange(vbuf->buf, strBase + (baseVtx * strStride), vtxCnt * strStride, AFX_BUF_MAP_R);

    for (afxNat i = 0; i < vtxCnt; i++)
        f(&src[i * strStride] + attrOffset, data);
...
_AFX afxError AfxVertexBufferDump(afxVertexBuffer vbuf, afxNat attrIdx, afxNat baseVtx, afxNat vtxCnt, void *dst, afxNat dstStride)
{
    ...
    for (afxNat i = 0; i < vtxCnt; i++)
    {
        AfxCopy(&dst2[i * dstStride], &src[i * strStride] + attrOffset, attrSiz);
    }
...
_AFX afxError AfxUpdateVertexBuffer(afxVertexBuffer vbuf, afxNat attrIdx, afxNat baseVtx, afxNat vtxCnt, void const *src, afxVertexFormat srcFmt, afxNat srcStride)
{
    ...
    AfxAssertRange(strRange, baseVtx * strStride, vtxCnt * strStride);
    afxByte* dst = AfxMapBufferRange(vbuf->buf, strBase + (baseVtx * strStride), vtxCnt * strStride, AFX_BUF_MAP_W);

    afxByte const* src2 = src;
    afxNat srcElemSiz = AfxVertexFormatGetSize(srcFmt);

    for (afxNat i = 0; i < vtxCnt; i++)
    {
        AfxCopy(&dst[i * strStride] + attrOffset, &src2[i * srcStride], attrSiz);
    }
...
_AFX afxNat AfxDescribeVertexStorage(afxVertexBuffer vbuf, afxNat secIdx, afxNat32* base, afxNat32* range, afxNat32* stride)
{
    ...
    if (base)
        *base = vbuf->storages[secIdx].base;

    if (range)
        *range = vbuf->storages[secIdx].range;
...
_AFX afxError AfxDescribeVertexAttribute(afxVertexBuffer vbuf, afxNat attrIdx, afxNat* secIdx, afxVertexFormat* fmt, afxNat32* offset, afxVertexUsage* usage)
{
    ...
    if (secIdx)
        *secIdx = vbuf->attrs[attrIdx].srcIdx;
