...
_AFX afxCmdId AfxCmdBindVertexSources(afxDrawScript dscr, afxNat baseSlot, afxNat slotCnt, afxBuffer buf[], afxNat32 const offset[], afxNat32 const range[], afxNat32 const stride[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dscr, afxFcc_DSCR);
    AfxAssertRange(8, baseSlot, slotCnt);
    return dscr->stdCmds->BindVertexSources(dscr, baseSlot, slotCnt, buf, offset, range, stride);
}
...
_AFX afxCmdId AfxCmdPackRasterRegion(afxDrawScript dscr, afxRaster ras, afxBuffer buf, afxRasterRegion const* rgn, afxSize bufOffset, afxNat bufRowSiz, afxNat bufRowCnt)
{
    ...
    AfxAssertRange(AfxGetBufferCapacity(buf), bufOffset, bufRowSiz * bufRowCnt);
...
_AFX afxCmdId AfxCmdFillBuffer(afxDrawScript dscr, afxBuffer buf, afxSize offset, afxSize range, afxNat value)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &dscr, afxFcc_DSCR);
    AfxAssertObjects(1, &buf, afxFcc_BUF);
