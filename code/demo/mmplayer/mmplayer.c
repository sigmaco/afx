...
void AfxCmdDrawText(afxDrawScript dscr, afxNat fontIdx, afxWhd origin, afxReal scale, afxString const* text)
{
    ...
    AfxCmdBindVertexSources(dscr, 0, 1, &textVbo, NIL, NIL);
    ...
    if (bnk->running)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                dpt.loadOp = afxSurfaceLoadOp_DONT_CARE;
...
_AFXEXPORT afxResult AfxEnterApplication(afxThread thr, afxApplication app)
{
    ...
    {
        surfSpec.surf = AfxGetDrawOutputSurface(dout[1], i);
    ...
    for (afxNat i = 0; i < doutConfig.bufCnt; i++)
    {
        surfSpec.surf = AfxGetDrawOutputSurface(dout[2], i);
    ...
    AfxLoadRastersFromTarga(dctx, afxRasterFlag_SAMPLING, 1, &uri2, &fontRas);
    AfxMakeUri(&uri2, "art/vcr-mono-font-256-left.tga", 0);
    AfxLoadRastersFromTarga(dctx, afxRasterFlag_SAMPLING, 1, &uri2, &fontRas2);
...
int main(int argc, char const* argv[])
{
    ...
    while (reboot)
    {
        ...
        sysConfig.base.dsys = &dsysConfig;
        AfxBootUpBasicIoSystem(&sysConfig.base, &sys);
        AfxAssertObjects(1, &sys, afxFcc_SYS);

        afxDrawDevice ddev;
        AfxGetDrawDevice(0, &ddev);
        AfxAssertObjects(1, &ddev, afxFcc_DDEV);

        afxDrawContextConfig dctxConfig = { 0 };        
        AfxAcquireDrawContexts(0, 1, &dctxConfig, &dctx);
