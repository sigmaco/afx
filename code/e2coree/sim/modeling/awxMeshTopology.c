...
_AFX afxError AfxBufferizeMeshTopology(awxMeshTopology msht)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
    awxMeshTopologyCache* cache = &msht->cache;

    if (!cache->buf)
    {
        awxSimulation sim = AfxGetObjectProvider(msht);
        AfxAssertObjects(1, &sim, afxFcc_SIM);

        afxNat idxSiz = AfxDetermineMeshIndexSize(msht);
        afxNat idxCnt = AwxCountMeshIndices(msht);
        ...
        if ((buf = cache->buf))
        {
            ...
            // debug
            void* p = AfxMapBufferRange(cache->buf, cache->base, cache->range, afxBufferAccess_R);
...
_AFX afxNat AfxDetermineMeshIndexSize(awxMeshTopology msht)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
#if 0
    afxNat surfCnt = msht->surfCnt;
    awxMeshSurface* surfaces = msht->surfaces;
    AfxAssert(surfCnt);
    afxNat triCnt = 0;

    for (afxNat i = 0; i < surfCnt; i++)
    {
        awxMeshSurface* surface = &surfaces[i];
...
_AFX afxIndexedTriangle* AwxGetMeshTriangles(awxMeshTopology msht, afxNat baseTriIdx)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
...
_AFX afxNat* AwxGetMeshIndices(awxMeshTopology msht, afxNat baseIdx)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
...
_AFX afxNat AwxCountMeshIndices(awxMeshTopology msht)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
...
_AFX afxNat AwxCountMeshTriangles(awxMeshTopology msht)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
    return msht->triCnt;
...
_AFX afxNat AwxCountMeshTriangleEdges(awxMeshTopology msht)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
...
_AFX void AwxGetMeshPrimitives(awxMeshTopology msht, afxNat surfIdx, afxNat* baseTriIdx, afxNat* triCnt)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
    AfxAssertRange(msht->surfCnt, surfIdx, 1);
    awxMeshSurface *mshs = &msht->surfaces[surfIdx];
...
_AFX afxBool AwxGetMeshCoverage(awxMeshTopology msht, afxNat surfIdx, afxNat* mtlIdx)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
    AfxAssertRange(msht->surfCnt, surfIdx, 1);
...
_AFX void AfxSetMeshCoverage(awxMeshTopology msht, afxNat surfIdx, afxNat mtlIdx)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
    AfxAssertRange(msht->surfCnt, surfIdx, 1);
...
_AFX awxMeshSurface* AwxGetMeshSurface(awxMeshTopology msht, afxNat surfIdx)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
    AfxAssertRange(msht->surfCnt, surfIdx, 1);
...
_AFX afxNat AwxCountMeshSurfaces(awxMeshTopology msht)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
    return msht->surfCnt;
...
_AFX void AfxInvertMeshWinding(awxMeshTopology msht)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
...
_AFX void AfxRemapMeshCoverage(awxMeshTopology msht, afxNat remapCnt, afxNat const remapTable[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
    ...
    {
        awxMeshSurface* mshs = &msht->surfaces[i];
        AfxAssertType(mshs, afxFcc_MSHS);
...
_AFX afxError _AfxMshtDtor(awxMeshTopology msht)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
...
_AFX afxError _AfxMshtCtor(awxMeshTopology msht, afxCookie const* cookie)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &msht, afxFcc_MSHT);
    AfxEntry("msht=%p", msht);
    ...
    if (surfCnt && !(msht->surfaces = AfxAllocate(mmu, sizeof(msht->surfaces[0]), surfCnt, 0, AfxHint()))) AfxThrowError();
    else
    {
        msht->tris = NIL;
        msht->triCnt = triCnt;

        if (triCnt && !(msht->tris = AfxAllocate(mmu, sizeof(msht->tris[0]), triCnt, 0, AfxHint()))) AfxThrowError();
        else
        {
            afxIndexedTriangle* tris = msht->tris;
            ...
            if (msht->vtxToVtxCnt && !(msht->vtxToVtxMap = AfxAllocate(mmu, sizeof(msht->vtxToVtxMap[0]), msht->vtxToVtxCnt, 0, AfxHint()))) AfxThrowError();
            else
            {
                msht->vtxToTriCnt = 0; // vtxCnt;
                msht->vtxToTriMap = NIL;

                if (msht->vtxToTriCnt && !(msht->vtxToTriMap = AfxAllocate(mmu, sizeof(msht->vtxToTriMap[0]), msht->vtxToTriCnt, 0, AfxHint()))) AfxThrowError();
                else
                {
                    msht->sideToNeighborCnt = 0;// triCnt * 3 * int; // totalIdxCnt;
                    msht->sideToNeighborMap = NIL;

                    if (msht->sideToNeighborCnt && !(msht->sideToNeighborMap = AfxAllocate(mmu, sizeof(msht->sideToNeighborMap[0]), msht->sideToNeighborCnt, 0, AfxHint()))) AfxThrowError();
                    else
                    {
...
_AFX awxMeshTopology AfxBuildMeshTopology(awxSimulation sim, awxMeshBuilder const* mshb, afxNat baseSurfIdx, afxNat surfCnt)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &sim, afxFcc_SIM);
    AfxAssert(mshb);
    awxMeshTopology msht = NIL;

    if (AfxAcquireObjects(AwxGetMeshTopologyClass(sim), 1, (afxObject*)&msht, (void const*[]) { sim, (void*)mshb, &baseSurfIdx, &surfCnt }))
        AfxThrowError();
...
_AFX afxClassConfig _AfxMshtClsConfig =
{
    ...
    .name = "Mesh Topology",
    .unitsPerPage = 8,
    .size = sizeof(AFX_OBJECT(awxMeshTopology)),
    .mmu = NIL,
