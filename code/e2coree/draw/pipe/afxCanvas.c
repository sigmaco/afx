...
_AFX afxBool AfxCanvasHasCombinedDepthStencil(afxCanvas canv)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &canv, afxFcc_CANV);
...
_AFX afxNat AfxCountColorSurfaces(afxCanvas canv)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &canv, afxFcc_CANV);
...
_AFX afxNat AfxGetSurfaceRasters(afxCanvas canv, afxNat baseSurf, afxNat surfCnt, afxRaster rasters[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &canv, afxFcc_CANV);
...
_AFX afxRaster AfxGetLinkedDepthRaster(afxCanvas canv)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &canv, afxFcc_CANV);
    afxRaster ras = NIL;
    ...
    {
        if (AfxGetSurfaceRasters(canv, surfIdx, 1, &ras))
        {
            AfxAssertObjects(1, &ras, afxFcc_RAS);
        }
    }
...
_AFX afxRaster AfxGetLinkedStencilRaster(afxCanvas canv)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &canv, afxFcc_CANV);
    afxRaster ras = NIL;
...
_AFX afxError AfxRelinkSurfaceRasters(afxCanvas canv, afxNat baseSurf, afxNat surfCnt, afxRaster rasters[])
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &canv, afxFcc_CANV);
...
_AFX afxError AfxRelinkDepthSurface(afxCanvas canv, afxRaster depth)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &canv, afxFcc_CANV);
    ...
    {
        if (AfxRelinkSurfaceRasters(canv, surfIdx, 1, &depth))
            AfxThrowError();
    }
    return err;
}

_AFX afxError AfxRelinkStencilSurface(afxCanvas canv, afxRaster stencil)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &canv, afxFcc_CANV);
    ...
    {
        if (AfxRelinkSurfaceRasters(canv, surfIdx, 1, &stencil))
            AfxThrowError();
    }
    return err;
}

_AFX afxError AfxGenerateSurfaceRasters(afxCanvas canv)
{
    afxError err = AFX_ERR_NONE;
    AfxAssertObjects(1, &canv, afxFcc_CANV);
    ...
    for (afxNat i = 0; i < canv->surfaceCnt; i++)
    {
        ...
        {
            if (sur->ras)
                AfxRelinkSurfaceRasters(canv, i, 1, NIL);
            ...
            texi.usage = sur->usage | afxRasterFlag_DRAW;
