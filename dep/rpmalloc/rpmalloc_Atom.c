...
#include <stdint.h>
//#include <stdatomic.h>
#define atomic_uint afxAtom32
#define atomic_ullong afxAtom64
#define atomic_uintptr_t afxAtomPtr

#if 0
#define atomic_store_explicit(_var_, _value_, _op_) ((void(__cdecl*)(__typeof(_var_),__typeof(_value_)))_Generic((_var_), afxAtom32*: AfxStoreAtom32, afxAtom64* : AfxStoreAtom64, afxAtomPtr* : AfxStoreAtomPtr))(_var_, _value_)
#define atomic_load_explicit(_var_, _op_) ((void(__cdecl*)(__typeof(_var_),__typeof(_value_)))_Generic((_var_), afxAtom32*: AfxLoadAtom32, afxAtom64* : AfxStoreAtom64, afxAtomPtr* : AfxStoreAtomPtr))(_var_, _value_)     _Generic(_var_, afxAtom32*: AfxLoadAtom32((_var_)), afxAtom64: AfxLoadAtom64((_var_)), afxAtomPtr: AfxLoadAtomPtr((_var_)))

//#define atomic_store_explicit(_var_, _value_, _op_) _Generic((_var_), afxAtom32*: AfxStoreAtom32((_var_),(_value_)), afxAtom64*: AfxStoreAtom64((_var_),(_value_)), afxAtomPtr*: AfxStoreAtomPtr((_var_),(_value_)))
//#define atomic_load_explicit(_var_, _op_) _Generic(_var_, afxAtom32*: AfxLoadAtom32((_var_)), afxAtom64: AfxLoadAtom64((_var_)), afxAtomPtr: AfxLoadAtomPtr((_var_)))
#define atomic_fetch_add_explicit(_var_, _value_, _op_) _Generic((_var_), afxAtom32*: AfxAddAtom32((_var_),(_value_)), afxAtom64*: AfxAddAtom64((_var_),(_value_)), afxAtomPtr*: AfxAddAtomPtr((_var_),(_value_)))
#define atomic_compare_exchange_weak_explicit(_var_, _expected_, _proposed_, _mode1_) _Generic((_var_), afxAtom32*: AfxCasAtom32((_var_),*(_proposed_),(_expected_)), afxAtom64*: AfxCasAtom64((_var_),*(_proposed_),(_expected_)), afxAtomPtr*: AfxCasAtomPtr((_var_),*(_proposed_),(_expected_)))

// _Bool atomic_compare_exchange_weak_explicit( volatile A *obj, C* expected, C desired, memory_order succ, memory_order fail );
// C     atomic_fetch_add_explicit( volatile A* obj, M arg, memory_order order ); // Return value = The value held previously by the atomic object pointed to by obj.
// C     atomic_load_explicit( const volatile A* obj, memory_order order ); // Return value = The current value of the atomic variable pointed to by obj.
...
page_full_to_free_on_new_heap(page_t* page, heap_t* heap) {
    ...
    AfxStoreAtom64(&page->thread_free, 0);
	page->next = heap->page_free[page->page_type];
	heap->page_free[page->page_type] = page;
	if (++heap->page_free_commit_count[page->page_type] > PAGE_FREE_OVERFLOW)
		heap_page_free_decommit(heap, page->page_type, PAGE_FREE_DECOMMIT);
...
page_adopt_thread_free_block_list(page_t* page) {
    ...
    unsigned long long thread_free = AfxLoadAtom64(&page->thread_free);
	if (thread_free != 0) {
		// Other threads can only replace with another valid list head, this will never change to 0 in other threads
		//while (!atomic_compare_exchange_weak_explicit(&page->thread_free, &thread_free, 0, memory_order_relaxed, memory_order_relaxed))
        while (!AfxCasAtom64(&page->thread_free, (afxInt64*)&thread_free, 0))
			wait_spin();
...
page_put_thread_free_block(page_t* page, block_t* block) {
    ...
    unsigned long long prev_thread_free = AfxLoadAtom64(&page->thread_free);
	...
	//while (!atomic_compare_exchange_weak_explicit(&page->thread_free, &prev_thread_free, thread_free, memory_order_relaxed, memory_order_relaxed))
    while (!AfxCasAtom64(&page->thread_free, (afxInt64*)&prev_thread_free, thread_free))
    {
	...
	if ((list_size == 1) && page->is_full) {
		...
		if (heap->id && heap->page_free_commit_count[page->page_type] < page->heap->page_free_commit_count[page->page_type]) {
            ...
            uintptr_t prev_head = (uintptr_t)AfxLoadAtomPtr(&heap->page_free_thread[page->page_type]);
			page->next = (void*)prev_head;
			//while (!atomic_compare_exchange_weak_explicit(&heap->page_free_thread[page->page_type], &prev_head, (uintptr_t)page, memory_order_relaxed, memory_order_relaxed))
            while (!AfxCasAtomPtr(&heap->page_free_thread[page->page_type], (void**)&prev_head, page))
            {
				page->next = (void*)prev_head;
				wait_spin();
			}
...
page_allocate_block(page_t* page, unsigned int zero) {
	...
	if (UNEXPECTED(block == 0)) {
        ...
        if (AfxLoadAtom64(&page->thread_free) != 0)
        {
...
heap_lock_acquire(void) {
    ...
    while (!AfxCasAtomPtr(&global_heap_lock, (void**)&lock, (void*)this_lock))
    {
		lock = 0;
		wait_spin();
	}
...
heap_lock_release(void) {
    ...
    AfxStoreAtomPtr(&global_heap_lock, 0);
}
...
heap_initialize(void* block) {
    ...
    heap->id = 1 + AfxAddAtom32(&global_heap_id, 1);
	return heap;
}
...
heap_get_page_generic(heap_t* heap, uint32_t size_class) {
    ...
    uintptr_t page_mt = (uintptr_t)AfxLoadAtomPtr(&heap->page_free_thread[page_type]);
	if (UNEXPECTED(page_mt != 0)) {
		//while (!atomic_compare_exchange_weak_explicit(&heap->page_free_thread[page_type], &page_mt, 0, memory_order_relaxed, memory_order_relaxed))
        while (!AfxCasAtomPtr(&heap->page_free_thread[page_type], (void**)&page_mt, 0))
        {
			wait_spin();
		}
		page = (void*)page_mt;
...
heap_free_all(heap_t* heap) {
	for (int itype = 0; itype < 3; ++itype) {
        ...
        AfxStoreAtomPtr(&heap->page_free_thread[itype], 0);
	}
...
#ifdef _WIN32
	fls_key = FlsAlloc((void*)&rpmalloc_thread_destructor);
#else
