...
{
    afxRasterFlag_LAYERED   = AFX_BIT(0), // has more than 1 layer/slice (or a set of them, in case of cubemap).
    afxRasterFlag_MIPMAP    = AFX_BIT(1), // has more than 1 LOD, which are subsamples from base image; (aka mipmaps).
    afxRasterFlag_RESAMPLED = AFX_BIT(2), // has more than 1 LOD, which are supersamples from base image; (aka multisample).
...
#if !0
    afxRasterFlag_ROW_MAJOR = AFX_BIT(9), // specifies linear tiling (texels are laid out in memory in row-major order, possibly with some padding on each row).
...
/*
    The AfxDescribeRasterFormat() function describes the raster's format capabilites in a given bridge.
*/

AVX avxFormat AfxDescribeRasterFormat(afxRaster ras, afxUnit exuIdx, avxFormatDescription* pfd);
...
AVX avxRange AfxGetRasterExtent(afxRaster ras, afxUnit lodIdx);
...
AVX void AfxGetRasterSwizzling(afxRaster ras, avxSwizzling* csw);
...
AVX afxError AfxPackRaster(afxRaster ras, afxUnit opCnt, afxRasterIo const ops[], afxUnit exuIdx, avxBuffer buf);
...
AVX afxError AfxUnpackRaster(afxRaster ras, afxUnit opCnt, afxRasterIo const ops[], afxUnit exuIdx, avxBuffer buf);
