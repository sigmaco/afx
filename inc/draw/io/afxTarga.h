...
#define AFX_TGA_HDR_SIZE (18 + \
    ((sizeof(afxUnit8[4]) + sizeof(afxUnit16[4]) + sizeof(afxTargaFlags) + sizeof(avxFormat)) + \
    (sizeof(afxUnit16[2]) + sizeof(afxUnit32[3]) + sizeof(afxUnit8[10])))) 
#define AFX_TGA_UDD_SIZE (255 - AFX_TGA_HDR_SIZE)

AFX_DEFINE_STRUCT(afxTargaFile)
{
    // TRUEVISION INC. FILE HEADER
    ...
    // SIGMA FEDERATION FILE HEADER EXTENSION
    ...
    afxUnit8        idd[9]; // originally it was just a padding to force AFX_SIMD_ALIGNMENT right after the header.
...
AFX_STATIC_ASSERT(AFX_IS_ALIGNED(AFX_TGA_HDR_SIZE, AFX_SIMD_ALIGNMENT), "");
AFX_STATIC_ASSERT(AFX_TGA_HDR_SIZE + AFX_TGA_UDD_SIZE == 255, "");
