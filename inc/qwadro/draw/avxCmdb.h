...
AVX afxError        AfxCompileCmdBuffers
(
    afxNat          cnt,
    avxCmdb         buffers[]  /// is the command buffer to complete recording.
);
...
AVX afxError        AfxRecycleCmdBuffers
(
    afxBool         freeRes, /// specifies that most or all memory resources currently owned by the command buffer should be returned to the parent command pool. If this flag is not set, then the command buffer may hold onto memory resources and reuse them when recording commands. @cmdb is moved to the initial state.
    afxNat          cnt,
    avxCmdb         buffers[] /// is the command buffer to reset. The command buffer can be in any state other than pending, and is moved into the initial state.
);

////////////////////////////////////////////////////////////////////////////////

/// AfxAcquireCmdBuffers can be used to allocate multiple command buffers.
...
AVX afxError        AfxAcquireCmdBuffers(afxDrawInput din, avxCmdbUsage usage, afxNat cnt, avxCmdb streams[]);

AVX afxError        AfxOpenCmdBuffers(afxDrawInput din, afxNat poolIdx, avxCmdbUsage usage, afxNat cnt, avxCmdb streams[]);
