...
AFX_DEFINE_STRUCT(afxSoundInfo)
{
    afxAudio        aud;
    afxTransform    origin;
    afxV3d          velocity;
...
ASX afxError    AfxDoSound(afxSoundSystem ssys, afxSoundInfo const* info, afxUnit* sndId);

ASX afxError    AfxStopSound(afxSoundSystem ssys, afxUnit sndId);

ASX void        AsxTransformSound(afxSound snd, afxTransform const* t);

ASX afxUnit     AsxAccumulateSoundSamples(afxSound snd, afxUnit sampleCnt, afxUnit chCnt, afxUnit freq, afxUnit bufCap, afxReal output[]);

ASX afxError    AsxRewindSound(afxSound snd);
ASX afxBool     AsxHasSoundEnded(afxSound snd);
